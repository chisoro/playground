...
*/
 FOR EACH iglabf NO-LOCK:
    ...
    PAUSE 0.
    FIND FIRST glbal WHERE glbal.acct = iglabf.acct-no AND glbal.YEAR = iglabf.bal-year NO-ERROR.
...
IF AVAILABLE muntmf THEN
    wstar = muntmf.tarif.
FOR EACH muncmf WHERE company = 0 AND acc > 90006300 AND acc < 5000000000 NO-LOCK:
...
FOR EACH munbmf WHERE company = 0 AND acc > 90006300 AND acc < 5000000000  NO-LOCK:
...
END.
*//*
FOR EACH munthf WHERE company = 0 AND acc <> 220252500  AND acc  > 90006300 AND acc < 5000000000:
