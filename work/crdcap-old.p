...
DEF    QUERY qry-Grn FOR stkgrn SCROLLING.
DEF BROWSE brw-Grn QUERY qry-Grn
     DISPLAY stkgrn.intOrder stkgrn.intGrn stkgrn.stkCode 
    stkgrn.decAmt stkgrn.decQty stkgrn.vat WITH 10 DOWN SEPARATORS.
...
DEFINE FRAME frm-Grn 
     brw-Grn AT ROW 1 COL 1.5 skip(0.2)
     btnOk COLON 8 SPACE (20) btnclose 
     WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER NO-VALIDATE
          SIDE-LABELS NO-UNDERLINE THREE-D SCROLLABLE CENTERED TITLE "Goods Received Notes Selection".
...
DO:
    IF DEC(wsOrd:SCREEN-VALUE) <> 0 THEN
        FIND FIRST ordmf WHERE Ordmf.acc = INT(wsAcc:SCREEN-VALUE) AND ordmf.OrdStat = 1 
...
DO:
  VIEW FRAM frm-alloc.
  ENABLE ALL EXCEPT wsGrn WITH FRAME frm-alloc.
  IF  wstype  <> 2 THEN
      DISABLE btnS WITH FRAME frm-Alloc.
  WAIT-FOR 'enter' OF wsGrn IN FRAME frm-alloc 
          OR CHOOSE OF btnL IN FRAME frm-alloc.
  HIDE FRAME frm-alloc.
  APPLY 'tab' TO btnLedger IN FRAME frm-allocate.
  APPLY 'tab' TO bfrCrdtaf.Ledger IN FRAME frm-allocate.
  RETURN.
END.
...
DO:
  wsOpt = "S".
  ENABLE wsGrn WITH FRAME frm-Alloc.
  APPLY 'entry' TO wsGrn IN FRAME frm-Alloc.
  RETURN NO-APPLY.
END.

ON 'tab':U OF wsGrn IN FRAME frm-Alloc
    OR 'enter':U OF wsGrn IN FRAME frm-Alloc
DO:
   ASSIGN wsGrn.
   IF wsGrn = NO THEN DO:
       VIEW FRAME frm-Store.
      OPEN QUERY qry-Store FOR EACH stkmf NO-LOCK.
      ENABLE ALL WITH FRAME frm-Store.
      WAIT-FOR CHOOSE OF btnClose IN FRAME frm-Store 
              OR close of THIS-PROCEDURE IN FRAME frm-Store
              OR CHOOSE OF btnok IN FRAME frm-Store 
              OR 'enter':u OF brw-Store
              OR 'mouse-select-dblclick' OF brw-Store.
      CLOSE QUERY qry-Store.
      DISABLE wsGrn WITH FRAME frm-Alloc. 
      HIDE FRAME frm-Store.
   END.
   ELSE IF wsgrn = YES THEN DO:
      VIEW FRAME frm-Grn.
      OPEN QUERY qry-Grn FOR EACH stkgrn 
          WHERE stkgrn.intOrder =  INT(wsOrd:SCREEN-VALUE IN FRAME frm-Input) NO-LOCK.
      ENABLE ALL WITH FRAME frm-Grn.
      WAIT-FOR CHOOSE OF btnClose IN FRAME frm-Grn 
              OR close of THIS-PROCEDURE IN FRAME frm-Grn
              OR CHOOSE OF btnok IN FRAME frm-Grn 
              OR 'enter':u OF brw-Grn
              OR 'mouse-select-dblclick' OF brw-Grn.
      CLOSE QUERY qry-Grn.
      DISABLE wsGrn WITH FRAME frm-Alloc. 
      HIDE FRAME frm-Grn.
   END.
...
DO:
    ...
    ELSE DO:
        ...
        ELSE DO:
            IF wsOpt = "S" THEN
                DISABLE btnDept btnProj bfrCrdtaf.Dept bfrCrdtaf.Proj WITH FRAME frm-allocate.
            ELSE DO:
               DISABLE btnDept btnProj  bfrCrdtaf.Dept  bfrCrdtaf.Proj WITH FRAME frm-allocate.
...
ON CHOOSE OF btnok IN FRAME frm-store 
    OR 'enter':u OF brw-store
    OR 'mouse-select-dblclick' OF brw-store
DO: 
   GET CURRENT qry-store NO-WAIT.
   ASSIGN wsWarehse = stkmf.WareHse
          wsStock   = stkmf.stkCode.
   FIND FIRST stkwhf WHERE stkwhf.WareHse = wsWarehse NO-LOCK no-error.
   DISPLAY stkwhf.Ledger @  bfrCrdtaf.Ledger stkwhf.Descrip @ glmf.DESCRIPTION 
       stkmf.Descrip @ bfrCrdtaf.descrip WITH FRAME frm-allocate.
END.

ON CHOOSE OF btnok IN FRAME frm-Grn 
    OR 'enter':u OF brw-Grn
    OR 'mouse-select-dblclick' OF brw-Grn
DO: 
   GET CURRENT qry-Grn NO-WAIT.
   ASSIGN wsWarehse = stkgrn.WareHse
          wsStock   = stkgrn.stkCode
          wsNote    = stkgrn.intGrn.
   FIND FIRST stkmf WHERE stkmf.stkCode = wsStock NO-LOCK NO-ERROR.
   FIND FIRST stkwhf WHERE stkwhf.WareHse = wsWarehse NO-LOCK NO-ERROR.
   ASSIGN NoteAmt = 0
          NoteVat = 0.
   FOR EACH bfrgrn WHERE bfrgrn.intGrn = wsNote:
       ASSIGN NoteAmt = NoteAmt + bfrgrn.decAmt
              NoteVat = NoteVat + bfrgrn.vat.
   END.
   DISPLAY stkwhf.Ledger @  bfrCrdtaf.Ledger stkwhf.Descrip @ glmf.DESCRIPTION 
       "GRN" + STRING(stkgrn.intGrn) @ bfrCrdtaf.descrip NoteAmt @ bfrCrdtaf.amount 
       NoteVat @ bfrCrdtaf.Vat WITH FRAME frm-allocate.
END.
...
DO:
  IF dec(bfrCrdtaf.Amount:SCREEN-VALUE) <> 0.00 
      ...
      ASSIGN tmpTrans.Acc      = wsAcc
             ...
             tmpTrans.trDate   = DATE(wsDate:SCREEN-VALUE IN FRAME frm-Input)
             tmpTrans.UID      = varUser.
      IF wsOpt = "S"  AND wsGrn = NO THEN DO:
          CREATE tmpStock.
          ASSIGN tmpStock.stkCode = wsStock
                 tmpStock.period  = INT(st-per:SCREEN-VALUE IN FRAME frm-Input)
                 tmpStock.PoDate  = TODAY
                 tmpStock.Quantity = DEC(wsQuant:SCREEN-VALUE)
                 tmpStock.Cost     = dec(bfrCrdtaf.Amount:SCREEN-VALUE) -  dec(bfrCrdtaf.Vat:SCREEN-VALUE)
                 tmpStock.Descrip  = bfrCrdtaf.descrip:SCREEN-VALUE IN FRAME frm-Allocate
                 tmpStock.Ledger   = 0
                 tmpStock.prog     = "crdcap.p"
                 tmpStock.Ref      = wsRef:SCREEN-VALUE IN FRAME frm-input
                 tmpStock.TransID  = wsTransId
                 tmpStock.TranType = 1
                 tmpStock.trDate   = DATE(wsDate:SCREEN-VALUE IN FRAME frm-Input)
                 tmpStock.UID      = varUser
                 tmpStock.UnitPrice = ROUND((dec(bfrCrdtaf.Amount:SCREEN-VALUE) / DEC(wsQuant:SCREEN-VALUE)),2).
      END.
      IF wsOpt = "S"  AND wsGrn = YES THEN DO:
          FOR EACH stkgrn WHERE stkgrn.intOrder = INT(wsOrd:SCREEN-VALUE IN FRAME frm-Input):
              ASSIGN stkgrn.InvDate  = DATE(wsDate:SCREEN-VALUE IN FRAME frm-Input)
                     stkgrn.Invoice  =  wsRef:SCREEN-VALUE IN FRAME frm-input
                     stkgrn.invoiced = YES.
          END.
      END.    
...
PROCEDURE Update-ip:
   ...
   FOR EACH tmpStock WHERE tmpStock.TransID = wsTransId:
       CREATE bfrStkTrans. /* Update and Create Stores transaction */
       BUFFER-COPY tmpStock TO bfrStkTrans.
       FIND FIRST stkmf WHERE stkmf.stkCode = bfrStkTrans.stkCode NO-ERROR.
       ASSIGN Stkmf.Quantity[2] = bfrStkTrans.Quantity
              Stkmf.cost[2]     = bfrStkTrans.Cost
              stkmf.UnitPrice   =  ROUND(((Stkmf.cost[1] + Stkmf.cost[2]) / (Stkmf.Quantity[1] 
                                           + Stkmf.Quantity[2])),2)
              stkmf.Cost[1]     = stkmf.Cost[1] + bfrStkTrans.Cost
              Stkmf.Quantity[1] = Stkmf.Quantity[1]+  bfrStkTrans.Quantity.
       DELETE tmpStock.
   END.
