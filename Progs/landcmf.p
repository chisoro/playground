...
&SCOPED-DEFINE wsTitle        "Account file Maintenance"
...
FORM dbgl.Proj          LABEL "PROJECT"
     ...
     wsDr               LABEL "DEBIT"
     wsCr               LABEL "CREDIT"
     HEADER skip(1) "                      LAND SALES/LEASE CONSOLIDATION REPORT FOR THE PERIOD:" AT 10 SPACE(2)
    STRING(wsPer)
...
define frame frm-input
    ...
    bfr{&tmptable}.LS COLON 30 LABEL "Type" SPACE(5)
   ...
   ELSE DO:
       ...
       ELSE DO:
           ...
           DISPLAY dbcmf.Name dbcmf.Add1 dbcmf.add2 dbcmf.Add3 wsDes  WITH FRAME frm-Input.
            APPLY 'entry' TO bfr{&tmptable}.ls IN FRAME  frm-Input.
       END.
   END.
   RETURN.    
END.
...
ON 'enter':U OF bfr{&tmpTable}.BillSep IN FRAME frm-input
    OR 'leave':U OF bfr{&tmpTable}.BillSep IN FRAME frm-input
DO:
    IF   bfr{&tmpTable}.BillSep:SCREEN-VALUE = "No" THEN
...
DO:
  ...
  OPEN QUERY qry-pickScheme FOR EACH LProj WHERE LProj.ls = INT(bfr{&tmptable}.ls:SCREEN-VALUE) NO-LOCK.
...
DO:
   FIND FIRST LProj WHERE LProj.Scheme = INT(bfr{&tmptable}.Scheme:SCREEN-VALUE)
       AND  LProj.ls = INT(bfr{&tmptable}.ls:SCREEN-VALUE) NO-LOCK NO-ERROR.
...
ON 'enter':U OF bfr{&tmptable}.ls 
DO:
    APPLY 'entry' TO bfr{&tmptable}.billSep.
    RETURN.
END.
...
DO:
     FIND FIRST {&tmptable} WHERE bfr{&tmptable}.Scheme = INT(bfr{&tmptable}.Scheme:SCREEN-VALUE)
         ...
         AND bfr{&tmptable}.ls     = INT(bfr{&tmptable}.ls:SCREEN-VALUE)
    ...
    IF (btn-ok:LABEL IN  FRAME frm-input) = "Save" THEN DO:
     ...
     FIND FIRST LProj WHERE LProj.Scheme  = INT(bfr{&tmptable}.Scheme:SCREEN-VALUE IN FRAME frm-input) NO-ERROR.
      ...
      ELSE DO:
          ...
          IF INT(bfr{&tmptable}.ls:SCREEN-VALUE) = 2 THEN DO:
              DO X = 1 TO 3:
