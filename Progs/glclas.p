/* Program.................GlClass.p
   Notes:................. Ledger ClasS  File Maintenance
   Author:.................S. Mawire
*/
session:DATA-ENTRY-RETURN = TRUE.
&SCOPED-DEFINE wsMsg           "Ledger ClasS  file already exist"
&SCOPED-DEFINE wsTitle           "Ledger ClasS File Masntenance"
&SCOPED-DEFINE tmptable             GlClass
&SCOPED-DEFINE skey                 GlClas.Clas
&SCOPED-DEFINE UpdFields      bfr{&tmptable}.DESCRIPTION ~
                                        COLUMN-LABEL ' Description ':C
&SCOPED-DEFINE tmpFields      bfr{&tmptable}.Clas ~
...
DO:
  IF (btn-ok:LABEL IN  FRAME frm-input) = "Save" THEN DO:
     FIND FIRST bfr{&tmptable} WHERE bfr{&tmptable}.Clas  = wsid NO-ERROR.
         IF AVAILABLE bfr{&tmptable} THEN DO:
           MESSAGE  "Ledger Clas already exist" VIEW-AS ALERT-BOX.
           RETURN NO-APPLY.
        END.
        ELSE DO:
            CREATE bfr{&tmptable}.
            ASSIGN bfr{&tmptable}.Clas = wsid
...
ON CHOOSE OF btn-Del DO:
    ...
    ASSIGN wsid = bfr{&tmptable}.Clas.
    FIND FIRST glmf WHERE glmf.Clas  = bfr{&tmptable}.Clas NO-ERROR.
    IF AVAILABLE glmf THEN DO:
        MESSAGE "Ledger Clas has related records - cannot be delete"
            VIEW-AS ALERT-BOX.
        RETURN NO-APPLY.
    END.
...
PROCEDURE proc-edit:
    GET CURRENT qry-{&tmptable} EXCLUSIVE-LOCK NO-WAIT.
    ASSIGN wsid = DEC(bfr{&tmptable}.Clas).
    FIND FIRST bfr{&tmptable} WHERE bfr{&tmptable}.Clas  = wsid NO-ERROR.
