...
DEF VAR wsAcAv AS INTEGER.
/*DEF  VAR wsSgrp LIKE dbhtf.sgrp. */
...
IF wsbut = "export" THEN DO:
    EXPORT STREAM b DELIMITER ',' "" "TOTALS" "" TAmt[1]  TAmt[2]  TAmt[3] 
                            TAmt[4]  TAmt[5]  TAmt[6].
END.
ELSE DO:
    UNDERLINE STREAM a dbcmf.dbacc dbcmf.NAME CurAmt[1] CurAmt[2] CurAmt[3] CurAmt[4] CurAmt[5] CurAmt[6]
                    WITH FRAME frm-rpt.
...
IF wsChoice = "C" THEN DO:
    DO X = 1 TO NUM-ENTRIES(wsSgrp) - 1:
       curAmt[1] = 0.
       curAmt[2] = 0.
        curAmt[3] = 0.
       curAmt[4] = 0.
       curAmt[5] = 0.
       j = INT(ENTRY(x,wsSgrp)).
       FOR each dbsgr WHERE dbsgr.sgrp = j NO-LOCK:
           wsSer = dbsgr.descrip.
       ...
       END.
         ...
         FOR EACH dbctf WHERE dbctf.cons >= st-Con AND dbctf.cons <= end-Con NO-LOCK.
               ...
               FOR EACH dbblf WHERE  dbblf.Sgrp = j  AND dbblf.dbacc >= wsStart AND dbblf.dbAcc <= wsEND NO-LOCK:
                   FOR EACH dbcmf WHERE dbcmf.dbAcc = dbblf.dbAcc AND dbcmf.cons = dbctf.cons AND dbcmf.AccStat = 0 NO-LOCK:
                     DISPLAY wsStatus WITH FRAME frm-main.
                     ASSIGN curAmt[1] = curAmt[1] + Amt[1] + Amt[2]
                            curAmt[2] = curAmt[2] + Amt[3]
                            curAmt[3] = curAmt[3] + Amt[4] + dbblf.Int
    ...
    ASSIGN curAmt[6] = CurAmt[1] + CurAmt[2] + CurAmt[3] + CurAmt[4] + CurAmt[5].
       IF wsBut = "Export" THEN
            EXPORT STREAM b DELIMITER ',' j  wsSer "" CurAmt[1] CurAmt[2] CurAmt[3] CurAmt[4] CurAmt[5] curAmt[6].
        ELSE DO:
             DISPLAY STREAM a j @ dbcmf.dbacc wsSer @ dbcmf.NAME CurAmt[1] CurAmt[2] CurAmt[3] CurAmt[4] CurAmt[5] curAmt[6]
                WITH FRAME frm-rpt.
            DOWN STREAM a WITH FRAME frm-rpt.
         END.
         ASSIGN TAmt[1] = TAmt[1] + curAmt[1]
...
ELSE IF wsChoice = "W" THEN DO:
   ...
   FOR EACH dbwrd WHERE dbwrd.ward >= st-ward AND dbwrd.ward <= end-ward NO-LOCK:
       ...
       FOR EACH dbblf WHERE  dbblf.Sgrp = j  AND dbblf.dbacc >= wsStart AND dbblf.dbAcc <= wsEND NO-LOCK:
         ...
         FOR EACH dbcmf WHERE dbcmf.dbAcc = dbblf.dbAcc AND dbcmf.ward = dbwrd.ward AND dbcmf.AccStat = 0 NO-LOCK.
            DISPLAY wsStatus WITH FRAME frm-main.
             ASSIGN curAmt[1] = curAmt[1] + Amt[1] + Amt[2]
                    curAmt[2] = curAmt[2] + Amt[3]
                    curAmt[3] = curAmt[3] + Amt[4] + dbblf.Int
...
ELSE IF wsChoice = "S" THEN DO:
   ...
   FOR EACH dbsmf WHERE dbsmf.suburb >= st-sub AND dbsmf.suburb <= end-sub NO-LOCK:  
       ...
       FOR EACH dbblf WHERE  dbblf.Sgrp = j  AND dbblf.dbacc >= wsStart AND dbblf.dbAcc <= wsEND NO-LOCK:
            FOR EACH dbcmf WHERE dbcmf.dbAcc = dbblf.dbAcc AND dbcmf.suburb = dbsmf.suburb AND dbcmf.AccStat = 0 NO-LOCK.
             DISPLAY wsStatus WITH FRAME frm-main.
             ASSIGN curAmt[1] = curAmt[1] + Amt[1] + Amt[2]
                    curAmt[2] = curAmt[2] + Amt[3]
                    curAmt[3] = curAmt[3] + Amt[4] + dbblf.Int
...
ELSE DO:
 DO X = 1 TO NUM-ENTRIES(wsSgrp) - 1:
       ...
       FOR EACH dbblf WHERE  dbblf.Sgrp = j  AND dbblf.dbacc >= wsStart AND dbblf.dbAcc <= wsEND NO-LOCK:
         FOR EACH dbcmf WHERE dbcmf.dbAcc = dbblf.dbAcc  AND dbcmf.AccStat = 0 NO-LOCK.
             DISPLAY wsStatus WITH FRAME frm-main.
             ASSIGN curAmt[1] = curAmt[1] + Amt[1] + Amt[2]
                    curAmt[2] = curAmt[2] + Amt[3]
                    curAmt[3] = curAmt[3] + Amt[4] + dbblf.Int
