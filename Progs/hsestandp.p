...
DEF VAR st-sub LIKE hsesch.scheme.
DEF VAR end-sub LIKE hsesch.scheme.
...
DEF VAR ppriceacc LIKE hsecmf.pprice.
DEF VAR ppriceaccgl LIKE hsecmf.pprice.
DEF VAR balacc LIKE hsecmf.bal.
DEF VAR balaccgl LIKE hsecmf.bal.
DEF VAR wsTitle2 AS CHAR.
...
DEF FRAME frm-main
    ...
    view-as combo-box size 30 by 4 list-item-pairs
      "All - All Accounts","A","W - By Ward","W","S - By Scheme","S"
    SKIP(0.5)
    st-sub      LABEL "Scheme FROM" COLON 30 SPACE(1)
    ...
    TITLE "STANDS REPORT" VIEW-AS DIALOG-BOX.
FORM 
     hsecmf.dbAcc      LABEL "ACCOUNT"
     hsesch.descrip       LABEL "SCHEME"      FORM "x(25)"
     hsecmf.StandNo        LABEL "STAND NUMBER"
     dbwrd.descrip        LABEL "WARD" FORM "x(15)"
     hsecmf.SIZE        LABEL "STAND SIZE"
    hsecmf.pprice       LABEL "PURCHASE PRICE"
    hsecmf.bal              LABEL "BALANCE"
     HEADER skip(1) "          STANDS REPORT AS AT: " AT 10 SPACE(2)
    STRING(TODAY)
...
DO:
   ...
   ASSIGN  wsStart wsEnd st-ward end-ward wsChoice  
            st-sub end-sub.
    EXPORT STREAM b DELIMITER ',' "STANDS REPORT".
    EXPORT STREAM b DELIMITER ',' "VACNT STANDS REPORT".
   RUN Report.ip.
  wsfile = "START " + wsFile.
...
IF wsChoice = "W" THEN DO:
    IF wsbut = "Export" THEN
       EXPORT STREAM b DELIMITER ',' "ACCOUNT" "SCHEME" "STAND NUMBER"  "SURBUB"  "WARD" "STAND SIZE" "PURCHASE PRICE" "BALANCE DUE".
    FOR EACH dbwrd WHERE dbwrd.ward >= st-ward AND dbwrd.ward <= end-ward NO-LOCK:
        wsTitle = "          WARD: - " + dbwrd.Descrip.
        wsTitle2 = "       Vacant Stands".
        IF wsbut = "Export" THEN
        ...
        FOR EACH hsecmf WHERE hsecmf.dbAcc >= wsStart AND hsecmf.dbAcc <= wsEnd 
            ...
            ppriceacc = ppriceacc + hsecmf.pprice.
            balacc = balacc + hsecmf.bal.
            wsStatus = string(hsecmf.dbacc).
           RUN idata.ip.
        END.
           DOWN STREAM a WITH FRAME frm-rpt.
           DISPLAY STREAM a ppriceacc @ hsecmf.pprice balacc @ hsecmf.bal WITH FRAME frm-rpt.
           PAGE STREAM a.
        END.
END.
ELSE IF wsChoice = "S" THEN DO:
    IF wsbut = "Export" THEN
       EXPORT STREAM b DELIMITER ',' "ACCOUNT" "SCHEME" "STAND NUMBER"  "SURBUB"  "WARD" "STAND SIZE" "PURCHASE PRICE" "BALANCE DUE".
    FOR EACH hsesch WHERE hsesch.scheme >= st-sub AND hsesch.scheme <= end-sub NO-LOCK:
        wsTitle = "          SCHEME: - " + HSESCH.Descrip.
        wsTitle2 = "       Vacant Stands".
        IF wsbut = "Export" THEN
            EXPORT STREAM b DELIMITER ',' "SCHEME " + HSESCH.Descrip.
        FOR EACH hsecmf WHERE hsecmf.dbAcc >= wsStart AND hsecmf.dbAcc <= wsEnd 
            AND hsecmf.sCHEME = hsesch.scheme AND hsecmf.AccStat <> 1 USE-INDEX acc NO-LOCK:
            ppriceacc = ppriceacc + hsecmf.pprice.
            balacc = balacc + hsecmf.bal.
           wsStatus = string(hsecmf.dbacc).
           RUN idata.ip.
...
ELSE DO:
  ...
  IF wsbut = "Export" THEN
   ...
   DISPLAY STREAM a ppriceacc @ hsecmf.pprice balacc @ hsecmf.bal WITH FRAME frm-rpt.
   END.
   ppriceaccgl = ppriceacc.
   balaccgl = balacc.
   IF wsBut = "Export" THEN
        EXPORT STREAM b DELIMITER ',' "STANDS ALLOCATED REPORT".
...
ELSE DO:
  ...
  IF wsbut = "Export" THEN
   ...
   DISPLAY STREAM a ppriceacc @ hsecmf.pprice balacc @ hsecmf.bal WITH FRAME frm-rpt.
   END.
   ppriceaccgl = ppriceaccgl + ppriceacc.
   balaccgl = balaccgl + balacc.
   DOWN STREAM a WITH FRAME frm-rpt.
   DISPLAY STREAM a ppriceaccgl @ hsecmf.pprice balaccgl @ hsecmf.bal WITH FRAME frm-rpt.
 END.
...
DISPLAY wsStatus WITH FRAME frm-main.
    IF wsBut = "Export" THEN
        EXPORT STREAM b DELIMITER ',' hsecmf.dbacc hsesch.descrip hsecmf.standno  dbwrd.descrip hsecmf.SIZE hsecmf.pprice hsecmf.bal.
    ELSE DO:
         DISPLAY STREAM a hsecmf.dbacc hsesch.descrip hsecmf.standno  dbwrd.descrip hsecmf.SIZE hsecmf.pprice hsecmf.bal
            WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt.
     END.
