...
/* Program.................payrpt07.p
    Notes:...... ..........Staff Terminations/Suspension listing Report
    Author:.................S. Mawire
*/
DEF STREAM a.
...
DEF VAR wsFilter AS INT
    VIEW-AS COMBO-BOX LIST-ITEM-PAIRS "All" , 0, "Suspension With Pay", 2, "Suspension Without Pay", 3.
...
DEFINE FRAME frm-pick 
 ...
 DEFINE FRAME frm-Paysys
     ...
     wsOpt     COLON 26 LABEL "Select Report" 
                VIEW-AS COMBO-BOX LIST-ITEM-PAIRS "SUSPENSION REPORT", 1, "TERMINATIONS REPORT", 2 
                SIZE 27 BY 2 SKIP(0.5)
     stDate    COLON 26 LABEL "From Date" SKIP(0.5)
     enDate    COLON 26 LABEL "Date Date" SKIP(0.5)
     wsFilter  COLON 26 LABEL "Filter Type" SKIP(3.0)
     btn-ok colon 20
     btn-close colon 60
 ...
 FORM 
     ...
     wsStatus       LABEL "STATUS"
     payemf.StatDate  LABEL "SUSPENDED"
 ...
 FORM 
     ...
     payemf.sex     LABEL "SEX"
     payemf.Grade   LABEL "GRADE" FORM "zz9"
     payemf.StartDate  LABEL "EMPLOYED" 
     payemf.StatDate  LABEL "TERMINATED" 
       HEADER wsCo "PAGE" AT 90 PAGE-NUMBER(a) SKIP(1) 
          wsTitle SKIP(2)
...
PROCEDURE Rpt.ip:
    wsTitle = "          "
             + "STAFF SUSPENSION LISTING BY DEPARTMENT FOR THE PERIOD: " + STRING(stDate) + " TO " + STRING(enDate).
    FOR EACH payemf WHERE Paysys = wsSys AND (estatus = 2 OR estatus = 3) NO-LOCK BREAK  BY dept BY StatDate :
        ...
        END.
        IF wsFilter <> 0 AND estatus <> wsFilter THEN NEXT.
        ASSIGN wsStatus = "WITH PAY" WHEN estatus = 2
               wsStatus = "WITHOUT PAY" WHEN estatus = 3.
        DISPLAY STREAM a payemf.empcode payemf.SName payemf.FName payemf.IDNo 
            payemf.sex payemf.Grade StatDate wsStatus WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt.
    END.
END.
...
PROCEDURE Rpt1.ip:
    wsTitle = "          "
            + "STAFF TERMINATIONS LISTING BY DEPARTMENT FOR THE PERIOD: " + STRING(stDate) + " TO " + STRING(enDate).
    FOR EACH payemf WHERE Paysys = wsSys AND estatus = 4 NO-LOCK BREAK  BY dept BY StatDate :
        IF FIRST-OF(dept) THEN DO:
            FIND FIRST paydept WHERE paydept.dept = payemf.dept NO-ERROR.
        ...
        END.
        DISPLAY STREAM a payemf.empcode payemf.SName payemf.FName payemf.IDNo 
            payemf.sex payemf.Grade payemf.StartDate StatDate WITH FRAME frm-rpt1.
        DOWN STREAM a WITH FRAME frm-rpt1.
    END.
END.
