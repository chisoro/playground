/* Program.................asrpt01.p
    Notes:...... ..........Asset Report listing Report
    Author:.................S. Mawire
    Modified:...............S. Chisoro
*/
...
DEF VAR wsPer  LIKE PAYSYS.PERIOD.
DEF VAR st-date AS DATE.
DEF VAR end-date AS DATE.
DEF VAR wsCo AS CHAR FORM "x(100)".
DEF VAR wsItem LIKE payitem.itemcode.
DEF VAR wsOpt AS INT VIEW-AS COMBO-BOX 
    LIST-ITEM-PAIRS "Asset Listing",1,
                    "Asset Listing by Category/Sub-category",2,
                    "Asset Listing by Sub-category",3,
                    "Asset Listing by Department/Sub-category",4,
                    "Asset Listing by Department",5.
DEF VAR wsTitle AS CHAR FORM "x(80)".
DEF VAR wsTitle1 AS CHAR FORM "x(100)".
...
DEFINE FRAME frm-Main
     SKIP(1.5)
     wsOpt     COLON 30 LABEL "SELECT REPORT" AUTO-RETURN SKIP(0.5)
     ...
     WITH 1 DOWN OVERLAY SIDE-LABELS CENTERED THREE-D 
      SIZE 95 BY 15
     TITLE "ASSET MASTERFILE LISTING REPORTS" VIEW-AS DIALOG-BOX.

FORM SPACE(5)
       Assmf.ACode COLUMN-LABEL "ASSET CODE" FORM "X(9)"
       Assmf.serial COLUMN-LABEL "SERIAL"
       Assmf.descrip COLUMN-LABEL "DESCRIPTION" FORM "X(26)"
       Assmf.PDate   COLUMN-LABEL "PURCHASE!DATE"
       Assmf.Cost    COLUMN-LABEL "COST"
       Assmf.Alife   COLUMN-LABEL "LIFE"
       Assmf.AccumDep COLUMN-LABEL "ACCUMULATED!DEPRECIATION"
       wsNbv          COLUMN-LABEL "NBV"
       HEADER SKIP(1) wsTitle AT 20 skip(2) wsTitle1 AT 5
            "Page:" AT 100 TRIM(STRING(PAGE-NUMBER(a))) SKIP(2)
...
DO:
    ASSIGN wsOpt st-cat st-subcat st-dept en-cat en-subcat en-dept.
...
PROCEDURE Report1.ip:
    ASSIGN wsTitle1 = "STANDARD ASSET MASTERFILE LISTING"
           wsCnt = 0
           tCost = 0
           tNbv  = 0
           tDep  = 0.
    FOR EACH assmf WHERE assmf.cat >= st-cat AND assmf.cat <= en-cat
                    ...
                    AND assmf.dept >= st-dept AND assmf.dept <= en-dept NO-LOCK:
        ASSIGN wsNbv = Assmf.Cost - Assmf.AccumDep
               tCost = tCost + assmf.cost
               tDep  = tDep + assmf.AccumDep
               tNbv  = tNbv + wsNbv
               wscnt = wscnt + 1.
        DISPLAY STREAM a Assmf.ACode Assmf.serial Assmf.descrip Assmf.PDate Assmf.Cost
            Assmf.Alife Assmf.AccumDep  wsNbv WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt.
    END.
    wsDes = "Total Assets = " + STRING(wscnt).
    UNDERLINE STREAM a assmf.descrip assmf.cost assmf.AccumDep wsNbv WITH FRAME frm-rpt.
    DISPLAY STREAM a  wsDes @ assmf.descri tCost @ assmf.cost 
        tDep @ assmf.AccumDep tNbv @ wsNbv WITH FRAME frm-rpt.
    DOWN STREAM a WITH FRAME frm-rpt.
END.

PROCEDURE Report2.ip:
    ASSIGN wsTitle1 =  "ASSET MASTERFILE LISTING BY CATEGORY BY SUB-CATEGORY"
           wsCnt = 0
           tCost = 0
           tNbv  = 0
           tDep  = 0.
...
PROCEDURE Report3.ip:
    ASSIGN wsTitle1 =  "ASSET MASTERFILE LISTING BY SUB-CATEGORY"
           wsCnt = 0
           tCost = 0
           tNbv  = 0
           tDep  = 0.
    FOR EACH assmf WHERE assmf.cat >= st-cat AND assmf.cat <= en-cat
        ...
        ASSIGN wsNbv = Assmf.Cost - Assmf.AccumDep
               ...
               tNbv  = tNbv + wsNbv
               wscnt = wscnt + 1.
        ACCUMULATE assmf.cost (TOTAL BY assmf.subcat).
        ACCUMULATE assmf.cost (COUNT BY assmf.subcat ).
        ACCUMULATE assmf.AccumDep (TOTAL BY assmf.subcat).
        ACCUMULATE Assmf.Cost - Assmf.AccumDep (TOTAL BY assmf.subcat).
        DISPLAY STREAM a Assmf.ACode "     " + Assmf.descrip @ Assmf.descrip Assmf.PDate Assmf.Cost
            Assmf.Alife Assmf.AccumDep  wsNbv WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt.
        IF LAST-OF(assmf.subcat) THEN DO:
...
PROCEDURE Report4.ip:
    ASSIGN wsTitle1 =  "ASSET MASTERFILE LISTING BY DEPARTMENT BY SUB-CATEGORY"
               wsCnt = 0
               tCost = 0
               tNbv  = 0
               tDep  = 0.
        FOR EACH assmf WHERE assmf.cat >= st-cat AND assmf.cat <= en-cat
                        AND assmf.subcat >= st-subcat AND assmf.subcat <= en-subcat
                        AND assmf.dept >= st-dept AND assmf.dept <= en-dept NO-LOCK
                        BREAK BY assmf.dept BY assmf.subcat :
            IF FIRST-OF(assmf.dept) THEN DO:
                FIND FIRST gldept WHERE gldept.dept = assmf.dept NO-LOCK NO-ERROR.
                IF AVAILABLE gldept THEN DO:
                    DISPLAY STREAM a "DEPARTMENT:" @ assmf.acode gldept.descrip @ assmf.Descrip WITH FRAME frm-rpt.
                    DOWN STREAM a  WITH FRAME frm-rpt.
                END.
            END.
            IF FIRST-OF(assmf.subcat) THEN DO:
            ...
            ASSIGN wsNbv = Assmf.Cost - Assmf.AccumDep
                   ...
                   tNbv  = tNbv + wsNbv
                   wscnt = wscnt + 1.
            ACCUMULATE assmf.cost (TOTAL BY assmf.dept BY assmf.subcat).
            ACCUMULATE assmf.cost (COUNT BY assmf.dept BY assmf.subcat ).
            ACCUMULATE assmf.AccumDep (TOTAL BY assmf.dept BY assmf.subcat).
            ACCUMULATE Assmf.Cost - Assmf.AccumDep (TOTAL BY assmf.dept BY assmf.subcat).
            DISPLAY STREAM a Assmf.ACode "     " + Assmf.descrip @ Assmf.descrip Assmf.PDate Assmf.Cost
                Assmf.Alife Assmf.AccumDep  wsNbv WITH FRAME frm-rpt.
            DOWN STREAM a WITH FRAME frm-rpt.
            IF LAST-OF(assmf.subcat) THEN DO:
                ...
                DOWN 2 STREAM a WITH FRAME frm-rpt.
            END. 
            IF LAST-OF(assmf.dept) THEN DO:
                FIND FIRST gldept WHERE gldept.dept = assmf.dept NO-LOCK NO-ERROR.
                wsDes =gldept.descrip + ":= " + STRING(ACCUM COUNT BY assmf.dept Assmf.Cost).
                UNDERLINE STREAM a assmf.descrip assmf.cost assmf.AccumDep wsNbv WITH FRAME frm-rpt.
                DISPLAY STREAM a  wsDes @ assmf.descrip 
                      ACCUM TOTAL BY assmf.dept assmf.cost  @ assmf.cost 
                      ACCUM TOTAL BY assmf.dept assmf.AccumDep @ assmf.AccumDep 
                      ACCUM TOTAL BY assmf.dept Assmf.Cost - Assmf.AccumDep @ wsNbv WITH FRAME frm-rpt.
                DOWN 2 STREAM a WITH FRAME frm-rpt.
            END. 
        END. 
END.

PROCEDURE Report5.ip:
    ASSIGN wsTitle1 =  "ASSET MASTERFILE LISTING BY DEPARTMENT"
           wsCnt = 0
           tCost = 0
           tNbv  = 0
           tDep  = 0.
    FOR EACH assmf WHERE assmf.cat >= st-cat AND assmf.cat <= en-cat
                    AND assmf.subcat >= st-subcat AND assmf.subcat <= en-subcat
                    AND assmf.dept >= st-dept AND assmf.dept <= en-dept NO-LOCK
                    BREAK BY assmf.dept BY assmf.syscode :
        IF FIRST-OF(assmf.dept) THEN DO:
            FIND FIRST gldept WHERE gldept.dept = assmf.dept NO-LOCK NO-ERROR.
            IF AVAILABLE gldept THEN DO:
                DISPLAY STREAM a "DEPARTMENT:" @ assmf.acode gldept.descrip @ assmf.Descrip WITH FRAME frm-rpt.
        ...
        ASSIGN wsNbv = Assmf.Cost - Assmf.AccumDep
               ...
               tNbv  = tNbv + wsNbv
               wscnt = wscnt + 1.
        ACCUMULATE assmf.cost (TOTAL BY assmf.dept).
        ACCUMULATE assmf.cost (COUNT BY assmf.dept ).
        ACCUMULATE assmf.AccumDep (TOTAL BY assmf.dept).
        ACCUMULATE Assmf.Cost - Assmf.AccumDep (TOTAL BY assmf.dept).
        DISPLAY STREAM a Assmf.ACode "     " + Assmf.descrip @ Assmf.descrip Assmf.PDate Assmf.Cost
            Assmf.Alife Assmf.AccumDep  wsNbv WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt.
        IF LAST-OF(assmf.dept) THEN DO:
            wsDes = "DEPARTMENT TOTAL = " + STRING(ACCUM COUNT BY assmf.dept Assmf.Cost).
            UNDERLINE STREAM a assmf.descrip assmf.cost assmf.AccumDep wsNbv WITH FRAME frm-rpt.
            DISPLAY STREAM a  wsDes @ assmf.descrip 
                  ACCUM TOTAL BY assmf.dept assmf.cost  @ assmf.cost 
                  ACCUM TOTAL BY assmf.dept assmf.AccumDep @ assmf.AccumDep 
