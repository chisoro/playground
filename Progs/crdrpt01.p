/* Program.................crdrpt01.p
   Notes:...... Creaditors Balances Report
   Author:.................S. Mawire
*/
...
DEF VAR wsTitle1  AS CHAR FORM "X(50)" INITIAL "CREDITORS ANALYSIS REPORT".
...
DEFINE FRAME frm-main
    ...
    TITLE "CREDITORS ANALYSIS REPORT".

FORM crdmf.acc NO-LABEL 
     Crdmf.Name   NO-LABEL FORM "x(40)"
     wsAmt        NO-LABEL 
     wsTotal      NO-LABEL
    HEADER SKIP(1) wsTitle0 AT 20 skip(1) wsDescrip AT 5 SKIP(3)
    wsTitle1 AT 5  "Page: " AT 66 TRIM(STRING(PAGE-NUMBER(a))) SKIP(2)
    "                                                         AMOUNT         BALANCE" SKIP(1)
        WITH DOWN STREAM-IO FONT 10 NO-LABEL CENTERED NO-BOX WIDTH 80 FRAME frmRpt.
...
PROCEDURE Rep.ip:
    wsTotal = 0.
    FOR EACH crdmf WHERE crdmf.acc >= wsAcc1 AND crdmf.acc <= wsAcc2 NO-LOCK:
        wsTotal = wsTotal + crdmf.Bal.
        DISPLAY crdmf.acc @ wsStatus WITH FRAME frm-main.
        PAUSE 0.
        DISPLAY STREAM a crdmf.acc crdmf.NAME crdmf.Bal @ wsAmt wsTotal WITH FRAME frmRpt.
