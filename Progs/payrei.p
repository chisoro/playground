...
/* Program.................payrei.p
    Notes:...... ..........Reinstate suspended/terminated employees.
    Author:.................S. Mawire
*/
DEF STREAM a.
...
DEFINE FRAME frm-pickJob 
 ...
 DEFINE FRAME frm-Paysys
     ...
     eStat      COLON 21.5 LABEL "Change status to" 
                VIEW-AS COMBO-BOX 
               LIST-ITEM-PAIRS "Re-Instate", 1, "Suspend with Pay", 2, "Suspend without Pay", 3 SKIP(0.5)
     wsDate     COLON 21.5 LABEL "Status Date" SKIP(0.5)
     btnJob               COLON 4  AUTO-RETURN NO-TAB-STOP
     payemf.post     NO-LABEL AUTO-RETURN
     PayPost.Descrip NO-LABEL VIEW-AS TEXT SKIP(1.5)
     btn-ok colon 20
     btn-close colon 60
     rect-1 AT ROW 1.5 COL 3
     rect-2 AT ROW 11.0 COL 3
     WITH 1 DOWN OVERLAY SIDE-LABELS CENTERED THREE-D 
      SIZE 95 BY 14
     TITLE "EMPLOYEE RE-INSTATEMENT " VIEW-AS DIALOG-BOX.
...
ON  'enter':U OF wsEmp IN FRAME frm-Paysys
 ...
 DO:
    ASSIGN wsEmp = INT(wsEmp:SCREEN-VALUE).
     ...
     IF AVAILABLE payemf AND payemf.eStat = 1 THEN DO:
         MESSAGE "Active employees cannot be re-instated...please try again" VIEW-AS ALERT-BOX.
         RETURN NO-APPLY.
     END.
     ELSE IF AVAILABLE payemf AND payemf.eStat <> 1 THEN DO:
         FIND FIRST paymtf WHERE paymtf.empcode = wsEmp AND paymtf.curAmt <> 0 NO-LOCK NO-ERROR.
         IF AVAILABLE paymtf THEN DO:
             MESSAGE "Process not allowed in an Open payroll" VIEW-AS ALERT-BOX.
             APPLY 'close' TO THIS-PROCEDURE.
         END.
          wsDes = payemf.SName + " " + payemf.FName.
          DISPLAY wsDes  payemf.StatDate @ wsDate WITH FRAME frm-Paysys.
          FIND FIRST Paypost WHERE PayPost.Post = payemf.post EXCLUSIVE-LOCK NO-ERROR.
          IF payemf.eStat <> 4 THEN
              DISABLE btnJob payemf.post WITH FRAME frm-Paysys.
          ELSE
              ENABLE btnJob payemf.post WITH FRAME frm-Paysys.
     END.
     ELSE DO:
 ...
 ON CHOOSE OF btnJob IN FRAME frm-Paysys
DO:
  VIEW FRAME frm-pickJob.
  OPEN QUERY qry-Job FOR EACH paypost WHERE PayPost.Vacant = YES NO-LOCK.
  ...
  APPLY 'tab' TO SELF IN FRAME frm-Paysys.
  APPLY 'tab' TO payemf.post.
  RETURN. 
END.
...
DO: 
   ...
   DISPLAY PayPost.Post @ payemf.Post PayPost.Descrip WITH FRAME frm-Paysys.
   APPLY 'tab' TO payemf.post. 
END.

ON 'tab':U OF  payemf.post IN FRAME frm-Paysys
    OR 'enter':U OF  payemf.post IN FRAME frm-Paysys
DO:
    FIND FIRST paypost WHERE paypost.post = INT(payemf.post:SCREEN-VALUE) EXCLUSIVE-LOCK NO-ERROR.
    IF AVAILABLE paypost THEN
        DISPLAY paypost.descrip WITH FRAME frm-Paysys.
    ELSE DO:
    ...
    CASE eStat:
        WHEN  1 THEN
            ASSIGN payemf.eStatus  = estat
                   ...
                   payemf.leaver   = NO
                   PayPost.Vacant  = NO
                   payemf.post = INT(payemf.post:SCREEN-VALUE).
        WHEN 2 THEN DO:
            ASSIGN payemf.eStatus = estat
                   payemf.leaver   = NO
                   PayPost.Vacant  = NO
