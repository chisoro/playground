/* Program.................gle01.p
   Notes:................. Ledger Enquiries by Project
   Author:.................S. Mawire
*/
...
DEF VAR wsProj   AS CHAR FORM "x(6)".
...
DEF VAR wsPYear  LIKE glPbal.YEAR.
DEF VAR wsPBf    LIKE glPbal.bfbal EXTENT 2.
DEF  VAR wsAvail LIKE glPbal.bfbal.
...
DEF QUERY qry-proj FOR glproj SCROLLING.
DEF BROWSE brw-proj QUERY qry-proj
    DISPLAY GlProj.Proj GlProj.DESCRIPTION WITH 10 DOWN SEPARATORS.

DEF FRAME frmProj
   brw-Proj AT ROW 2 COL 5
    skip(1.5)
    btn-ok COLON 5
    btn-close COLON 20
...
DEF FRAME frmEnquiry
     ...
     btnProj AT ROW 3 COL 5  NO-TAB-STOP SPACE(3)
     wsProj NO-LABEL  AUTO-RETURN   
     glProj.description NO-LABEL NO-TAB-STOP VIEW-AS FILL-IN SIZE 40 by 1
...
DEF FRAME frmEnquiry        
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER NO-VALIDATE
         ...
         SIZE 98 BY 26 TITLE "LEDGER ENQUIRIES BY PROJECT".
...
ON CHOOSE OF btnProj IN FRAME frmEnquiry
DO:
  VIEW FRAME frmProj.
  OPEN QUERY qry-Proj FOR EACH glProj NO-LOCK.
  ENABLE ALL WITH FRAME frmProj.
  WAIT-FOR CHOOSE OF btn-close IN FRAME frmProj 
          OR close of THIS-PROCEDURE IN FRAME frmProj
          OR CHOOSE OF btn-ok IN FRAME frmProj 
          OR 'enter':u OF brw-Proj
          OR 'mouse-select-dblclick' OF brw-Proj.
  CLOSE QUERY qry-Proj.
  HIDE FRAME frmProj.
...
ON CHOOSE OF btn-ok IN FRAME frmProj 
    OR 'enter':u OF brw-Proj
    OR 'mouse-select-dblclick' OF brw-Proj
DO: 
   GET CURRENT qry-Proj NO-LOCK NO-WAIT.
   DISPLAY glProj.Proj @ wsProj glProj.DESCRIPTION WITH FRAME frmEnquiry.
   APPLY 'TAB' TO wsProj IN FRAME frmEnquiry.
END.

ON 'tab' of wsProj IN FRAME frmEnquiry
    OR 'enter' OF wsProj IN FRAME frmEnquiry
DO:
   ASSIGN wsProj = wsProj:SCREEN-VALUE. 
   FIND FIRST glproj WHERE glProj.Proj = DEC(wsProj:SCREEN-VALUE) NO-LOCK NO-ERROR.
    IF NOT AVAIL glProj then
    do:
        wsAn = YES.
        MESSAGE "Project number" wsProj "does not exist." SKIP
                "Get next available Project number?."
            VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO 
                        UPDATE wsAn.
        IF wsAn THEN DO:
            FIND FIRST glProj NO-LOCK NO-ERROR.
        END.
        IF NOT AVAIL glProj THEN RETURN.
    end.
    DISPLAY glProj.Proj @ wsProj glProj.DESCRIPTION WITH FRAME frmEnquiry.
END.
...
DO:
    ...
    FIND FIRST glPbal WHERE glPbal.YEAR = INT(wsYear:SCREEN-VALUE) - 1 AND glPbal.acct = glmf.acct 
                        AND GLPBal.Proj = INT(wsProj:SCREEN-VALUE) NO-LOCK NO-ERROR.
    IF AVAILABLE glPbal THEN DO:
        ASSIGN wsPreA = 0
               wsPreB = 0.
        DO X = 1 TO 12:
            ASSIGN wsPreA[X] = glPbal.amt[X]
                   wsPreB[X] = glPbal.budget[X]
                   wsPreA[13] = wsPreA[13] + glPbal.amt[X]
                   wsPreB[13] = wsPreB[13] + glPbal.budget[X].
        END.
        ASSIGN wsPbf[2] = glPbal.BFBAL
    ...
    ELSE ASSIGN wsPreA = 0
                wsPreB = 0.
    FIND FIRST glPbal WHERE glPbal.YEAR = INT(wsYear:SCREEN-VALUE) AND glPbal.acct = glmf.acct 
                        AND GLPBal.Proj = INT(wsProj:SCREEN-VALUE)NO-LOCK NO-ERROR.
    IF AVAILABLE glPbal THEN DO:
         ASSIGN wsCurA = 0
                wsCurB = 0.
        DO X = 1 TO 12:
            ASSIGN wsCurA[X] = glPbal.amt[X]
                   wsCurB[X] = glPbal.budget[X]
                   wsCurA[13] = wsCurA[13] + glPbal.amt[X]
                   wsCurB[13] = wsCurB[13] + glPbal.budget[X].
        END.
        ASSIGN wsPbf[1] = glPbal.BFBAL WHEN glPbal.BFBAL <> 0
...
PROCEDURE ViewTrans.ip:
  ...
  OPEN QUERY qry-trans FOR EACH gltdf WHERE gltdf.acct = glmf.acct AND gltdf.period >= st-per 
                            AND gltdf.proj = int(wsProj) NO-LOCK.
