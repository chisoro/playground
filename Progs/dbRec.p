...
DEF VAR dbRate LIKE tblForex.decRate FORM "zzzzzzzzz9.9999".
DEF VAR rdbRate LIKE tblForex.decRate FORM "zzzzzzzzz9.9999".
DEF VAR wsRate LIKE tblForex.decRate FORM "zzzzzzzzz9.9999".
...
DEFINE VARIABLE w_password AS CHARACTER FORMAT "X(20)".  /*AS CHARACTER FORMAT "X(8)":U  */
...
DEFINE FRAME frm-input 
    ...
    wsReRate    LABEL "RECEIPTING USD RATE"  VIEW-AS TEXT  FONT 6 skip(0.5)
    ...
    SKIP(1)
    btn-exit COLON 40 NO-TAB-STOP  SPACE(50) btnprn SPACE(40) btnClear
...
DO:
    ...
    ASSIGN  varRecNo = varRecNo + 1
            varDue  = 0
            X = 0.
    DISPLAY varRecNo varDate wsdisRate wsReRate WITH FRAME frm-input.
   ...
   ENABLE varTend WITH FRAME frm-input.
    ...
    IF AVAILABLE dbPay THEN DO:
        ...
        ASSIGN varCur  = cbkmf.txtCur
               /*wsMark  = "P;" + SUBSTR(dbpay.descrip,1,8)*/.
...
IF wsValid = YES THEN DO:
    ...
    ENABLE ALL EXCEPT varRef varTend WITH FRAME frm-input.
    CLEAR FRAME frm-input ALL.
    wsRecDetails = "".
    varMeth:SCREEN-VALUE = "1".
    varloop:SCREEN-VALUE = "N".
    DISPLAY varRecNo varDate  varCur wsDisRate wsReRate WITH FRAME frm-input.
                ...
                IF AVAILABLE bfrDBrecCtr AND bfrDbRecCtr.RecDate <> TODAY THEN DO:
                    ...
                    IF choice THEN DO:
                        ...
                        FIND FIRST simusr WHERE simusr.usercode = wsSup:SCREEN-VALUE 
                            ...
                            NO-LOCK NO-ERROR.
                        MESSAGE wsSup  sup-password simusr.NAME VIEW-AS ALERT-BOX.
    ...
    ELSE IF varType = "L" THEN DO:
        ...
        CREATE tblDetail.
          ...
          /*FOR EACH dbrec WHERE dbRec.Account = varAcc AND dbRec.RecStat = "" AND dbRec.contype = "L" NO-LOCK:
              FIND FIRST tblForex WHERE tblForex.txtCur = dbrec.txtCur AND dbrec.recdate >= tblForex.DtRate  NO-LOCK NO-ERROR.
                IF AVAILABLE tblForex THEN DO:
                      ASSIGN wsRate = tblForex.decRate.
                END.
                ELSE IF NOT AVAILABLE tblForex  THEN DO:
                   FIND LAST tblForexH WHERE tblForexH.txtCur = dbrec.txtCur AND  tblForexH.dtRate <= dbrec.recdate  NO-LOCK NO-ERROR.
                   IF AVAILABLE tblForexH THEN DO:
                       ASSIGN wsRate = tblForexH.decRate.
                    END.
                END.
...
PROCEDURE rec-prn.ip:
    ...
    FOR EACH tmpbfrDbRec:
        ...
        ELSE wsTax = "A".
        /* Masvingo */
        IF simctr.cocode = "COM" AND tmpbfrdbrec.txtcur <> "ZWL" THEN DO:
    ...
    IF simctr.COCODE = "CVF" AND varType  = "C" /*CVF */
        AND (tmpbfrDbRec.Account < 51000000 OR tmpbfrDbRec.Account > 71999999) THEN /* City of Victoria Falls Discount */
          ASSIGN wsDisc = ROUND((varTotal * (10 / 100)),2) WHEN cbkmf.txtCur = "USD". /*CVF */
