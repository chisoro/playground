...
DEF BUTTON btn-Next  LABEL "NEXT" SIZE 16.5 BY 1.5.
DEF BUTTON btn-Prev  LABEL "PREVIOUS" SIZE 16.5 BY 1.5.
DEF BUTTON btn-Add   LABEL "TRANSACTIONS" SIZE 16.5 BY 1.5.
...
define frame frm-Enq
    ...
    btn-ok AT ROW 15.2 COL 10 SPACE(18) btn-Next SPACE(18) 
    btn-Prev SPACE(18) btn-Add SPACE(18) btn-close  SKIP(0.5)
    with view-as dialog-box keep-tab-order no-validate
         SIDE-LABELS no-underline three-d SCROLLABLE BGCOLOR 11 FGCOLOR 1 CENTERED
    TITLE "ASSET ENQUIRIES".

define frame frm-ADEnq
    SKIP(0.5)
    ...
    btn-Next AT ROW 12.5 COL 10  SPACE(18) 
    btn-Prev SPACE(18) btn-close  SKIP(0.5)
    with view-as dialog-box keep-tab-order no-validate
         SIDE-LABELS no-underline three-d SCROLLABLE BGCOLOR 11 FGCOLOR 1 CENTERED
    TITLE "Accessaries/Improvements Asset Details".

/* ***** Triggers for the main frame **** */
ON 'enter':U OF  Assmf.ACode IN FRAME frm-Enq
    OR 'tab':U OF  Assmf.ACode IN FRAME frm-Enq
DO:
    FIND FIRST assmf WHERE assmf.aCode =  Assmf.ACode:SCREEN-VALUE AND assmf.Astatus = "A" NO-LOCK NO-ERROR.
    IF NOT AVAILABLE assmf THEN DO:
        MESSAGE "No Asset found, please try again..." VIEW-AS ALERT-BOX.
        CLEAR FRAME frm-Enq.
        RETURN NO-APPLY.
    END.
    ELSE DO:
        ...
        DISPLAY assmf.Descrip asubcat.descrip assmf.Alife assmf.Dper wsDMethod
                ...
                GlDept.DESCRIP glfund.descrip WITH FRAME frm-Enq.
...
ON 'choose':U OF btn-ok IN FRAME frm-Enq
DO:
    CLEAR FRAME frm-Enq.
...
ON CHOOSE OF btn-next IN FRAME frm-Enq
DO:
    
    FIND NEXT assmf WHERE assmf.Astatus = "A" NO-LOCK NO-ERROR.
    IF NOT AVAILABLE  assmf THEN
        FIND LAST  assmf WHERE assmf.Astatus = "A" NO-LOCK NO-ERROR.
    assmf.acode:SCREEN-VALUE = assmf.acode.
    APPLY 'tab' TO assmf.acode.
    RETURN.
END.

ON CHOOSE OF btn-Prev IN FRAME frm-Enq
DO:
    
    FIND PREV assmf WHERE assmf.Astatus = "A"  NO-LOCK NO-ERROR.
    IF NOT AVAILABLE  assmf THEN
        FIND FIRST  assmf WHERE assmf.Astatus = "A" NO-LOCK NO-ERROR.
    assmf.acode:SCREEN-VALUE = assmf.acode.
    APPLY 'tab' TO assmf.acode.
    RETURN.
END.

ON CHOOSE OF BtnAss IN FRAME frm-Enq
    OR CHOOSE OF BtnAss IN FRAME frm-Enq
DO:
...
DO: 
   ...
   assmf.Acode:SCREEN-VALUE IN FRAME frm-Enq = assmf.Acode.
   APPLY 'tab' TO btnASS.
   APPLY 'tab' TO assmf.ACode IN FRAME frm-Enq.
   RETURN. 
END.
ON 'choose':U OF btn-Add IN FRAME frm-Enq
DO:
...
ON 'mouse-select-dblclick':U OF  brw-PickImp IN FRAME frm-Enq
    OR 'enter':u OF  brw-PickImp IN FRAME frm-Enq
DO:
   GET CURRENT qry-PickImp  NO-LOCK NO-WAIT.
    VIEW FRAME frm-AdEnq.
    DISPLAY asadf WITH FRAME frm-AdEnq.
    IF  asubcat.DMethod = 1 THEN DO:
        ...
        END.
    ENABLE btn-Next Btn-Prev Btn-Close WITH FRAME frm-AdEnq.
    WAIT-FOR CHOOSE OF btn-close OR close of THIS-PROCEDURE IN FRAME frm-AdEnq.
    HIDE FRAME frm-AdEnq.
    RETURN.
END.

ON CHOOSE OF btn-next IN FRAME frm-AdEnq
DO:
    FIND NEXT asadf NO-LOCK NO-ERROR.
    IF NOT AVAILABLE  asadf THEN
        FIND LAST  asadf NO-LOCK NO-ERROR.
    DISPLAY asadf WITH FRAME frm-AdEnq.
    IF  asubcat.DMethod = 1 THEN DO:
            asadf.Alife:VISIBLE = TRUE.
            asadf.Dper:VISIBLE = FALSE.
...
ON CHOOSE OF btn-Prev IN FRAME frm-AdEnq
DO:
    FIND PREV asadf NO-LOCK NO-ERROR.
    IF NOT AVAILABLE  asadf THEN
        FIND FIRST  asadf NO-LOCK NO-ERROR.
    DISPLAY asadf WITH FRAME frm-AdEnq.
    IF  asubcat.DMethod = 1 THEN DO:
            asadf.Alife:VISIBLE = TRUE.
            asadf.Dper:VISIBLE = FALSE.
...
/********** MAIN LOGIC **********/
VIEW FRAME frm-Enq.
ENABLE  btnAss Assmf.ACode btn-ok brw-PickImp btn-Next btn-Prev btn-Add btn-close WITH FRAME frm-Enq.
WAIT-FOR CHOOSE OF btn-close OR close of THIS-PROCEDURE IN FRAME frm-enq.
HIDE FRAME frm-main.
