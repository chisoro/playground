...
PROCEDURE Exc1.ip:
    ...
    ASSIGN wsAmt = 0
           curAmt = 0
           ytdAmt = 0.
    EXPORT STREAM B DELIMITER ',' "                         TRIAL BALANCE REPORT FOR THE PERIOD:" + STRING(wsYear) + STRING(wsMonth,"99").
    EXPORT STREAM b DELIMITER ',' "" "" " --------CURRENT PERIOD--------" ""  " --------TOTAL TO DATE--------".
    EXPORT STREAM b  DELIMITER ',' "ACCOUNT" "DESCRIPTION" "DEBITS" "CREDITS" "DEBITS" "CREDITS".
   FOR EACH glmf WHERE glmf.acct >= wsStart AND glmf.acct <= wsEnd NO-LOCK BY glmf.acct:
       ASSIGN wsAmt[1] = 0
              curAmt[1] = 0
              curAmt[2] = 0
       ...
       ELSE IF AVAILABLE glbal THEN DO:
          ...
          IF ytdAmt[1] <> 0 OR ytdAmt[2] <> 0 THEN DO:
             EXPORT STREAM b  DELIMITER ','
                 glmf.acct UPPER(glmf.DESCRIPTION)  curAmt[1]  curAmt[2] ytdAmt[1] ytdAmt[2] SKIP.
          END.
           
       END.
   END.
   EXPORT  STREAM b DELIMITER ',' "TOTALS" ""
                        curAmt[3] curAmt[4] ytdAmt[3] ytdAmt[4] SKIP.
   EXPORT  STREAM b DELIMITER ',' "DIFF" "" ""
       (curAmt[3] + curAmt[4]) "" (ytdAmt[3] + ytdAmt[4]) SKIP.
    RETURN.
END.
        ...
        IF CAN-FIND (FIRST glPbal WHERE glPbal.proj = glproj.proj AND glPbal.YEAR = wsYear)  THEN DO:
           ...
           FOR EACH glPbal WHERE glPbal.proj = glproj.proj AND glPbal.YEAR = wsYear
               ...
               ASSIGN wsAmt[1] = glPbal.amt[wsMonth]
                   ...
                   END.
           END. /* each glPbal */
           EXPORT  STREAM b DELIMITER ',' "PROJECT TOTALS" "" pTAmt[1] pTAmt[2] pTAmt[3] pTAmt[4] .
        END.
    END. /* each glproj */
    EXPORT  STREAM b DELIMITER ',' "GRAND TOTALS" "" curAmt[3] curAmt[4] ytdAmt[3]  ytdAmt[4].
    EXPORT  STREAM b DELIMITER ',' "DIFF" "" "" (curAmt[3] + curAmt[4]) "" (ytdAmt[3] + ytdAmt[4]) .
    RETURN.
END.
...
PROCEDURE Exc3.ip:
    ...
    FOR EACH glfund WHERE glfund.fund >= st-fund AND glfund.fund <= end-fund:
        IF CAN-FIND (FIRST glFbal WHERE glFbal.fund = glfund.fund AND glFbal.YEAR = wsYear)  THEN DO:
           ...
           END. /* each glFbal */
           EXPORT  STREAM b DELIMITER ',' "SEGMENT TOTALS" "" DtAmt[1] DtAmt[2] DtAmt[3] DtAmt[4]. 
        END.
    END. /* each glFund */
    EXPORT  STREAM b DELIMITER ',' "GRAND TOTALS" "" curAmt[3]  curAmt[4] ytdAmt[3]  ytdAmt[4].
    EXPORT  STREAM b DELIMITER ',' "DIFF" "" " " (curAmt[3] + curAmt[4]) " " (ytdAmt[3] + ytdAmt[4]) .
...
PROCEDURE Exc4.ip:
    ...
    FOR EACH gldept WHERE gldept.dept >= st-dept AND gldept.dept <= end-dept BY dept:
        IF CAN-FIND (FIRST gldbal WHERE gldbal.dept = gldept.dept AND gldbal.YEAR = wsYear)  THEN DO:
           ...
           END. /* each glDBal */
            EXPORT  STREAM b DELIMITER ',' "DEPARTMENT TOTALS" "" DtAmt[1] DtAmt[2] DtAmt[3] DtAmt[4].
        END.
    END. /* each gldept */
    EXPORT  STREAM b DELIMITER ',' "GRAND TOTALS" "" curAmt[3] curAmt[4] ytdAmt[3] ytdAmt[4].
    EXPORT  STREAM b DELIMITER ',' "DIFF" " " "" (curAmt[3] + curAmt[4]) "" (ytdAmt[3] + ytdAmt[4]) .
