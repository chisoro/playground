...
PROCEDURE Report.ip:
    ASSIGN wsTitle1 =  "ASSET DISPOSAL REPORT BY CATEGORY BY SUB-CATEGORY FOR THE PERIOD "
    ...
    FOR EACH assmf WHERE assmf.cat >= st-cat AND assmf.cat <= en-cat
                    ...
                    AND assmf.DDate >= st-date AND assmf.DDate <= end-date  NO-LOCK
        ...
        DISPLAY STREAM a Assmf.ACode "     " + Assmf.descrip @ Assmf.descrip Assmf.PDate Assmf.Cost
                Assmf.DDate assmf.DPrice Assmf.AccumDep  wsNbv WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt.
