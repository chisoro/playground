/******************************************************************************
  ...
  11/10/03    G Campbell    Copied pdf_inc.i to pdf_inc.p and reworked pdf_inc.i
                            ...
                            {1} - can be nothing, THIS-PROCEDURE, or non-blank
  ...
  08/15/12    JC Cardot     add parameter {2} to specify a "version" of pdf_inc.p
                            to test with
  
  05/11/14    JC Cardot     if we found pdfinc but it is not a super proc of the session,
                            or this-procedure, then add it ; see samples/super/layer.p
                            This allows to use the same pdf_inc.p instance in many .p procedures.
                            The first to be run must use the "THIS-PROCEDURE" argument of pdf_inc.i
                            while these others must use no argument.
  
  10/16/14    JC Cardot     move old versions of pdfinc.p in the old/ subdirectory
  
  03/26/15    JC Cardot     move pdfglobal.i to inc
  
******************************************************************************/

{ inc/pdfglobal.i "NEW SHARED"}

/* PEKI - Find the handle if on the air */
&IF "{1}" = "THIS-PROCEDURE" &THEN
    /* h_PDFinc = THIS-PROCEDURE. */ /* 13-MAY-2014 jcc: does not work, will not find any running pdf include procedure. We have to use SESSION:FIRST-PROCEDURE. */
...
IF NOT VALID-HANDLE(h_PDFinc) THEN DO:
  ...
  &IF "{2}" = "" &THEN
  RUN {&PDFDIR}pdf_inc.p PERSISTENT 
                SET h_PDFinc.
  &ELSE
  RUN {&PDFDIR}old/pdf_inc.{2}.p PERSISTENT 
                SET h_PDFinc.
  &ENDIF
...
ELSE DO:
    /* 11-MAY-2014 jcc: if we found pdfinc but it is not a super proc, then add it ; see samples/super/layer.p */
    IF LOOKUP(STRING(h_PDFinc), SESSION:SUPER-PROCEDURES) = 0 AND
       LOOKUP(STRING(h_PDFinc), THIS-PROCEDURE:SUPER-PROCEDURES) = 0 THEN
