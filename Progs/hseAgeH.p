...
DO:
   ...
   wsBut = "Print".
    wsAmt = 0.
    wsTot = 0.
   ASSIGN  wsStart wsEnd  wszero wsper.
...
PROCEDURE Report.ip:
FOR EACH hsecmf WHERE hsecmf.dbacc >= wsStart AND hsecmf.dbacc <= wsEnd NO-LOCK:
    DISPLAY hsecmf.dbacc @ wsStatus WITH FRAME frm-main.
    PAUSE 0.
    ASSIGN wsAmt = 0
           wsRec = 0.
   FOR EACH hsehtf WHERE hsehtf.dbacc = hsecmf.dbacc
                         AND hsehtf.accper <= wsPer NO-LOCK: 
            IF hsehtf.amt < 0 THEN
                ASSIGN wsrec = wsrec + hsehtf.amt
                       wsAmt[5] = wsAmt[5] + hsehtf.amt.
            ELSE IF hsehtf.amt > 0 THEN DO:
                IF hsehtf.Accper = wsper THEN
                    wsAmt[1] = wsAmt[1] + hsehtf.amt.
                ELSE IF hsehtf.Accper = (wsper - 1) THEN
                    wsAmt[2] = wsAmt[2] + hsehtf.amt.
                ELSE IF hsehtf.Accper = (wsper - 2) THEN
                    wsAmt[3] = wsAmt[3] + hsehtf.amt.
                ELSE IF hsehtf.Accper = (wsper - 3) THEN
                    wsAmt[4] = wsAmt[4] + hsehtf.amt.
                ELSE /*IF hsehtf.Accper <= (wsper - 4) THEN*/ 
                    wsAmt[6] = wsAmt[6] + hsehtf.amt.
                wsAmt[5] = wsAmt[5] + hsehtf.amt.
            END.
    END.  /* eo each hsehtf */
    wsRec = wsRec * -1.
    ...
    IF wsBut = "Print"  THEN DO:
        /* FIND FIRST hseblf WHERE hseblf.dbacc = hsecmf.dbacc NO-ERROR.
        IF AVAILABLE hseblf THEN DO:
            ASSIGN hseblf.amt[1] = wsamt[1]
                   hseblf.amt[2] = wsamt[2]
                   hseblf.amt[3] = wsamt[3]
                   hseblf.amt[4] = wsamt[4]
                   hseblf.amt[5] = wsamt[6].
        END.
        IF NOT AVAILABLE HSEBLF THEN DO:
            CREATE hseblf.
            ASSIGN hseblf.dbacc = hsecmf.dbacc
                   hseblf.Scheme = hsecmf.scheme
                   hseblf.amt[1] = wsamt[1]
                   hseblf.amt[2] = wsamt[2]
                   hseblf.amt[3] = wsamt[3]
                   hseblf.amt[4] = wsamt[4]
                   hseblf.amt[5] = wsamt[6].
        END. */
        DISPLAY STREAM a hsecmf.dbacc hsecmf.NAME wsAmt[1] wsAmt[2] wsAmt[3] wsAmt[4] wsAmt[6] wsAmt[5]
                WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt.
    END.
    ELSE
        EXPORT STREAM b DELIMITER "," hsecmf.dbacc hsecmf.NAME wsAmt[1] wsAmt[2] wsAmt[3] wsAmt[4] wsAmt[6] wsAmt[5].
   
END. /* eo-each hsecmf */
IF wsBut = "Print"  THEN DO:
    UNDERLINE STREAM a wsAmt[1] wsAmt[2] wsAmt[3] wsAmt[4] wsAmt[6] wsAmt[5]
            WITH FRAME frm-rpt.
    DISPLAY STREAM a "TOTAL" @ hsecmf.NAME 
        wsTot[1] @ wsAmt[1]
        wsTot[2] @ wsAmt[2]
        wsTot[3] @ wsAmt[3]
        wsTot[4] @ wsAmt[4]
