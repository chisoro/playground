...
/* Program.................paycap02.p
   Notes:...... Capture standard amount for all employees
   Author:.................S. Mawire
*/
...
DEFINE FRAME frm-Data
    ...
    rec1 AT ROW 7.4   COL 2
    WITH 1 DOWN OVERLAY SIDE-LABELS CENTERED THREE-D SIZE 73 BY 10
    TITLE "STANDARD AMOUNT DATA CAPTURE - ALL EMPLOYEES" VIEW-AS DIALOG-BOX.
...
DO:
    ...
    FOR EACH payemf WHERE payemf.paysys = wsSys AND (payemf.eStatus = 1 OR  payemf.eStatus = 2):
        ...
        FIND FIRST paymtf WHERE paymtf.empcode = payemf.empcode AND paymtf.itemcode = wsitem NO-ERROR.
            ...
            END.
            ASSIGN Paymtf.StdAmt   = wsAmt WHEN Paymtf.ACTIVE = 1.
            IF wsitem = Payctr.Basic AND Paymtf.ACTIVE = 1 THEN DO:
                DO X = 1 TO 3: /*Overtime items */
                       IF Payctr.overtime[X] <> 0 THEN DO:
                           FIND FIRST bfrpaymtf WHERE bfrpaymtf.empcode = payemf.empcode AND bfrpaymtf.itemcode = Payctr.overtime[X] NO-ERROR.
                            IF NOT AVAILABLE bfrpaymtf THEN DO:
                               CREATE bfrpaymtf.
                               ASSIGN bfrpaymtf.empcode = payemf.empcode
                                      bfrPaymtf.ACTIVE   = 1
                                      bfrPaymtf.Itemcode = Payctr.overtime[X].
                            END.
                            bfrPaymtf.StdAmt   = DEC(wsAmt:SCREEN-VALUE) / payemf.stdhours
                                                * Payctr.ORate[X]. 
                       END.
                END.
            END.
