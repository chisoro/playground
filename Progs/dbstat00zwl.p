...
FORM                                              /* tran line */
    varDate   AT 5
     varRef   AT 14              
     varDesc  AT 26
     varAmt   AT 75 FORM  "zzzzzzzzz9.99-" SPACE(0)
     varVats 
     with width 132
	  no-box
	  no-labels
...
form
     ...
     varAcc                 at 49
     varSgrp                at 70 FORM ">>"
     varAmt                 at 75 FORM  "zzzzzzzzz9.99-"
     with width 132
          no-box
          no-labels
          down
...
FORM
     ...
     ""    AT 70
     SKIP(1)
     wsAge[1]      AT 4 FORM  "zzzzzz9.99-" 
     wsage[5]      AT 16 FORM  "zzzzzz9.99-" 
     wsage[4]      AT 27 FORM  "zzzzzz9.99-" 
     wsage[3]      AT 38 FORM  "zzzzzz9.99-" 
     wsage[2]      AT 49 FORM  "zzzzzz9.99-"     
     wsAge[6]      AT 75 FORM  "zzzzzzzzz9.99-" 
     SKIP(1)
     wsmessage1    AT 5         
     wsmessage2    AT 5
     /*SKIP(1)*/
...
IF AVAILABLE tblForex THEN
   ...
   ASSIGN  wsStart = DEC(wsStart:SCREEN-VALUE)
           ...
           wsMessage1 = "DISCLAIMER: THE ABOVE BILL IS AN EQUIVALENCE OF THE OBTAINING AT PREVAILING RATE".
           wsMessage2 = "ON THE DATE OF BILLING WHICH IS USD1:ZWL" + STRING(WSrATE).
...
FOR EACH dbcmf WHERE dbcmf.dbacc >= wsStart AND dbcmf.dbacc <= wsEnd NO-LOCK.
    ...
    IF  dbcmf.AccBal = 0 /*AND wsZero = NO */ THEN NEXT.
        ASSIGN  varDue  = 0
                ...
                wsCr    = 0
                j       = 0.
    DISPLAY STREAM a dbcmf.dbAcc @ wsAcc wsDueDate wsAccDate   /*wsPeriod */ dbcmf.stno dbcmf.street
    ...
    END.
    wsage[1] = wsage[1] * wsRate.
        wsage[2] = wsage[2] * wsRate.
        wsage[3] = wsage[3] * wsRate.
        wsage[4] = wsage[4] * wsRate.
        wsage[5] = wsage[5] * wsRate.
        wsage[6] = wsage[6] * wsRate.
    wsmeter = "".
    ...
    DOWN STREAM a WITH FRAME frm-hdr.
    DISPLAY STREAM a "BALANCE B/F " @ varDesc (dbcmf.BalBf * wsRate) @ varAmt
    ...
    IF AVAILABLE dbblf THEN DO:
         ...
         DISPLAY STREAM a  dbblf.sgrp @ varSgrp (varAmt * wsRate) @ varAmt WITH FRAME frm-stub.
...
PROCEDURE rep-ip:
    ...
    IF AVAILABLE tmpwork THEN DO:
       ...
       DISPLAY STREAM a tmpwork.trDate @ varDate tmpwork.ref @ varRef 
              upper(tmpwork.DESCRIP) @ varDesc varAmt (varAmt * wsRate) @ varAmt  "*" WHEN tmpwork.vat <> 0 @ varVats  WITH FRAME frm-trans.
       DOWN STREAM a WITH FRAME frm-trans.
       j = j + 1.
    END.
    REPEAT:
