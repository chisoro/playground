/* Program.................dbCBot.p
   Notes:................. Program to create pdf statements for chatbot upload/ works with a python interface to connect to ftp site and upload bills
                                First part checks for dupicated tarrif accounts then create a list for review. Listed accounts will be skipped on all digital platforms i.e. emial/web inquiry and chatbot
   Author:.................S. Chisoro
*/
...
DEF VAR wsChoice1 AS CHAR FORM "x(1)".
...
DEF VAR wsMessage3 AS CHAR.
DEF VAR wsmail AS CHAR FORM "X(40)".
...
DEF FRAME frm-main
    SKIP(1.5)
    wsChoice1 LABEL "Screen By?" COLON 30
    VIEW-AS COMBO-BOX SIZE 30 BY 4 LIST-ITEM-PAIRS
    "A - All Accounts","A","E - With Emails Only", "E"
    SKIP(0.5)
    ...
    wsDueDate  LABEL "Last Receipt Date"                   SKIP(0.5)
     ...
     wsmail LABEL "Reply To" COLON 15 SKIP(0.5)
     wsmessage3 LABEL "Email Body"  VIEW-AS EDITOR SIZE 80 BY 6 COLON 15 SKIP 
    SKIP(0.5)
    ...
    TITLE "DIGITAL PLATFORM STATEMENTS PROCESSING" VIEW-AS DIALOG-BOX.
...
ON 'tab':U OF wsChoice1 
    OR 'enter' OF wsChoice1
DO:
    ASSIGN wsChoice1 = wsChoice1:SCREEN-VALUE.
 RETURN.
END.
...
DO:
   ...
   OUTPUT STREAM c TO VALUE(wsFile3).
   ...
   OUTPUT STREAM c CLOSE.
   MESSAGE "Please correctDOUBLE TARRIFED ACCOUNTS in file doubleTarrif.csv. These where not send to digital platforms." VIEW-AS ALERT-BOX.
      /*OS-COMMAND NO-WAIT VALUE(wsFile3).*/
    OS-COMMAND NO-WAIT VALUE(wsFile2 + "bin\digitise.exe"). 
/*Command for either chatbot or email*/
...
ASSIGN st-con:SCREEN-VALUE = "0"
       ...
       wsChoice1:SCREEN-VALUE = "A"
       wsSgrp:SCREEN-VALUE = "".

wsAccPer = simctr.curper.
wsmail:SCREEN-VALUE  =  simctr1.email.
FILE-INFO:FILE-NAME = ".".
wsFile = simctr.repDir + "Statements.csv".
...
PROCEDURE checktar.ip:
    EXPORT STREAM c DELIMITER ',' "Account"    "Tarrif".
   ...
   END.
    ...
    FOR EACH dbtmf WHERE dbtmf.type = 3:
            ...
            DO i = 1 to num-entries(sTar):
                wsb = STRING(dbtmf.tarif).
                IF (entry(i,sTar)) = wsb THEN DO:
                    n = n + 1.
                END.
            END.
            IF n > 1 THEN DO:
...
PROCEDURE writetoFile.ip:
    FOR EACH tmpDTarrif:
        EXPORT STREAM c DELIMITER ',' tmpDTarrif.dbAcc    tmpDTarrif.tarif.
    END.
END PROCEDURE.
...
PROCEDURE report.ip:
    ...
    IF wsChoice = "C" THEN DO: /* by Consumer */
        FOR EACH dbctf WHERE dbctf.cons >= st-Con AND dbctf.cons <= end-Con NO-LOCK.
            FOR EACH dbcmf WHERE dbcmf.dbAcc >= wsStart AND dbcmf.dbAcc <= wsEnd 
                AND dbcmf.cons = dbctf.cons AND dbcmf.AccStat = 0 /*AND dbcmf.emailAdd <> ""*/ USE-INDEX dbacc NO-LOCK:
               wsStatus = string(dbcmf.dbacc).
               DISPLAY wsStatus WITH FRAME frm-main. PAUSE 0.
               FIND FIRST tmpConsum WHERE tmpConsum.dbacc = dbcmf.dbacc NO-LOCK NO-ERROR.
               IF AVAILABLE tmpConsum THEN DO:
                   RUN DataExtract-ip.
               END.
            END.
        END.
    ...
    ELSE IF wsChoice = "W" THEN DO:  /* by ward */
        FOR EACH dbwrd WHERE dbwrd.ward >= st-ward AND dbwrd.ward <= end-ward NO-LOCK:
           FOR EACH dbcmf WHERE dbcmf.dbAcc >= wsStart AND dbcmf.dbAcc <= wsEnd 
                AND dbcmf.ward = dbwrd.ward AND dbcmf.AccStat = 0 /*AND dbcmf.emailAdd <> ""*/ USE-INDEX dbacc NO-LOCK:
               wsStatus = string(dbcmf.dbacc).
                DISPLAY wsStatus WITH FRAME frm-main. PAUSE 0.
               FIND FIRST tmpConsum WHERE tmpConsum.dbacc = dbcmf.dbacc NO-LOCK NO-ERROR.
               IF AVAILABLE tmpConsum THEN DO:
    ...
    ELSE IF wsChoice = "S" THEN DO: /* by surbub */
        FOR EACH dbsmf WHERE dbsmf.suburb >= st-sub AND dbsmf.suburb <= end-sub NO-LOCK:
            FOR EACH dbcmf WHERE dbcmf.dbAcc >= wsStart AND dbcmf.dbAcc <= wsEnd 
                AND dbcmf.suburb = dbsmf.suburb AND dbcmf.AccStat = 0 /*AND dbcmf.emailAdd <> ""*/ USE-INDEX dbacc NO-LOCK:
               wsStatus = string(dbcmf.dbacc).
                DISPLAY wsStatus WITH FRAME frm-main. PAUSE 0.
               FIND FIRST tmpConsum WHERE tmpConsum.dbacc = dbcmf.dbacc NO-LOCK NO-ERROR.
               IF AVAILABLE tmpConsum THEN DO:
    ...
    ELSE DO:
        FOR EACH dbcmf WHERE dbcmf.dbAcc >= wsStart AND dbcmf.dbAcc <= wsEnd 
           AND dbcmf.AccStat = 0 /*AND dbcmf.emailAdd <> ""*/ USE-INDEX dbacc NO-LOCK: /* AND dbcmf.cell <> 0*/
           wsStatus = string(dbcmf.dbacc).
            DISPLAY wsStatus WITH FRAME frm-main. PAUSE 0.
            FIND FIRST tmpConsum WHERE tmpConsum.dbacc = dbcmf.dbacc NO-LOCK NO-ERROR.
               IF AVAILABLE tmpConsum THEN DO:
    ...
    FIND FIRST simctr NO-LOCK NO-ERROR.
    FIND FIRST simctr1 NO-LOCK NO-ERROR.
   EXPORT STREAM a DELIMITER ','  SIMCTR.CONAME SIMCTR.Add1 SIMCTR.add2 SIMCTR.Add3   SIMCTR.REGNO  SIMCTR.Phone wsMessage3:SCREEN-VALUE IN FRAME frm-main simctr1.serveradd simctr1.serverport simctr1.serverpass simctr1.serveremail simctr.BPNO simctr1.email.
