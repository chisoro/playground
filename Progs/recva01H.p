...
FORM bfdbRecH.RecNo      LABEL "RECEIPT"
     bfdbRecH.SeqNo      LABEL "SEQ"
     bfdbRecH.Account    LABEL "ACCOUNT"
     bfdbRecH.Descrip      LABEL "DESCRIPTION"
     bfdbRecH.Paytype    LABEL "PCODE"
     bfdbRecH.rCode      LABEL "ICODE"
     BFdbRecH.Amount     LABEL "AMOUNT"
     wsTotal            LABEL "Rcpt Total"
...
FIND FIRST bfdbRecH WHERE bfdbRecH.OpCode = wsOp AND bfdbRecH.RecDate = wsDate EXCLUSIVE-LOCK NO-ERROR.
...
PROCEDURE Report.ip.
   FOR EACH bfdbRecH where bfdbRecH.OpCode = wsOp AND bfdbRecH.RecDate = wsDate NO-LOCK BREAK BY RecNo :
       wsStatus = "".
   ...
   PAGE STREAM a.
   DISPLAY STREAM a "COLLECTION BY INCOME CODES" @ bfdbRecH.descrip WITH FRAME frm-rpt.
   DOWN 2 STREAM a WITH FRAME frm-rpt.
   FOR EACH bfdbRecH WHERE bfdbRecH.OpCode = wsOp AND bfdbRecH.RecDate = wsDate AND bfdbRecH.Recstat <> "C"
       ...
       IF LAST-OF(bfdbRecH.rCode) THEN DO:
           wsVar = bfdbRecH.rCode. 
           DISPLAY STREAM a wsVar @ bfdbRecH.descrip wsTotal WITH FRAME frm-rpt.
           DOWN STREAM a WITH FRAME frm-rpt.
       END.      
   END.
   UNDERLINE STREAM a wsTotal WITH FRAME frm-rpt.
   ...
   FOR EACH tblforex:
        ...
        ASSIGN wsTotal = 0.
        FOR EACH bfdbRecH WHERE bfdbRecH.OpCode = wsOp AND bfdbRecH.RecDate = wsDate
