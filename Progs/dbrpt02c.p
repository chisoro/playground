...
DEF VAR wsLedger LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR wsVari LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR wsAmt  LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR wsBal  LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR wsRec LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR balcd LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR wsColl LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR wsTAmt LIKE wsAmt.
DEF VAR wsTLAmt LIKE wsAmt.
DEF VAR wsVat  LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
...
DEF VAR wsLAmt LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR wsTBalcd LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR wsBalbd  LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-". 
DEF VAR wsTBalbd LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-". 
DEF VAR tdeduct LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-". 
DEF VAR deduct LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR wsBalcd LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR wsRecAmt LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR wsjnlAmt  LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR wsTjnlAmt LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".
DEF VAR  wsTRecAmt LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-".

DEF TEMP-TABLE tbAna
    FIELD tbward LIKE dbcmf.ward
    FIELD tbcons LIKE dbcmf.cons
    FIELD tbsgrp LIKE dbmtf.sgrp
    FIELD tbtarif LIKE dbmtf.tarif
    FIELD tbcnt  AS INT
    FIELD tbRate LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-"
    FIELD tbVat% LIKE dbtmf.vat%
    FIELD tbAmt LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-"
    FIELD tbVat LIKE dbmtf.vat FORM "zzz,zzz,zz9.99-"
    FIELD tbRecAmt LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-"
    FIELD tbRecVat LIKE dbmtf.vat FORM "zzz,zzz,zz9.99-"
    FIELD tbjnlAmt LIKE dbmtf.amt FORM "zzz,zzz,zz9.99-"
    FIELD tbjnlVat LIKE dbmtf.vat FORM "zzz,zzz,zz9.99-".
...
PROCEDURE Report.ip:
     ...
     FOR EACH dbmtf  NO-LOCK:
            FIND FIRST dbcmf WHERE dbcmf.dbacc = dbmtf.dbacc NO-LOCK NO-ERROR.
            FIND FIRST tbAna WHERE  tbCons = dbcmf.Cons AND tbSgrp = dbmtf.sgrp AND tbTarif = dbmtf.tarif AND tbWard = dbcmf.ward NO-ERROR.
                ...
                IF NOT AVAILABLE tbAna THEN DO:
                    ...
                    ASSIGN tbWard = dbcmf.ward 
                           ...
                           tbTarif = dbmtf.tarif.
                END.
                IF dbmtf.prog = "dbsg07.p" THEN DO:
                        FIND FIRST dbtmf WHERE dbtmf.sgrp = dbmtf.sgrp AND dbtmf.Tarif = dbmtf.tarif NO-LOCK NO-ERROR.
                        IF AVAILABLE dbtmf THEN   
                            ASSIGN tbRate =  dbtmf.charge
                                   tbVat% = dbtmf.vat%.
                    
                        ASSIGN tbCnt = tbCnt + 1
                               tbAmt = tbAmt + dbmtf.amt
                               tbVat = tbVat + dbmtf.vat.
                    END.
                ELSE IF dbmtf.prog = "recupd.p" THEN DO:
                        ASSIGN 
                               tbRecAmt = tbRecAmt + (dbmtf.amt * -1)
                               tbRecVat = tbRecVat + (dbmtf.vat * -1). 
                END.
                ELSE DO:
                
                    ASSIGN tbjnlAmt = tbjnlAmt + dbmtf.amt
                           tbjnlVat = tbjnlVat + dbmtf.vat.
                 END.
                 /*ELSE DO:
                    FIND FIRST tbAna WHERE  tbCons = 0 AND tbSgrp = dbmtf.sgrp AND tbTarif = dbmtf.tarif AND tbWard = 0 NO-ERROR.
                     IF NOT AVAILABLE tbAna THEN DO:
                            CREATE tbAna.
                            ASSIGN tbWard = 0 
                                   tbCons = 0
    ...
    FOR EACH dbsgr NO-LOCK:
       ...
       FOR EACH  dbblf WHERE dbblf.sgrp = dbsgr.sgrp NO-LOCK.
        ...
        FOR EACH  dbmtf WHERE dbmtf.sgrp = dbsgr.sgrp AND dbmtf.accper > st-per NO-LOCK.
                 deduct = deduct + dbmtf.amt.
        ...
        IF (wsAmt <> 0 OR wsjnlAmt <> 0 OR wsRecAmt <> 0) THEN DO: 
            ...
            wsBalbd = wsBalbd - wsAmt - wsjnlAmt + wsRecAmt.
