...
define frame frm-main
    ...
    "ASSET DISPOSAL DETAILS" AT ROW 1 COL 110 SKIP(1)
    wsPer          COLON 125 LABEL "Accounting Period" SPACE(10) wsDate LABEL "Transaction Date" SKIP(0.5)
    "Reason for Disposal" COLON 110 SKIP(0.2)
    wsDes           COLON 110 NO-LABEL  FORM "x(60)" SKIP(1)
    wsAmt           COLON 125 LABEL "Amount Realised" SKIP(1)
    wsLedger        COLON 125 LABEL "Disposal Ledger"
...
ASSIGN wsper:SCREEN-VALUE = STRING(simctr.CURPER) 
       ...
       wsSource = "AS".
VIEW FRAME frm-main.
ENABLE  btnAss Assmf.ACode btn-ok btn-close wsamt wsLedger wsDes wsDate wsPer WITH FRAME frm-main.
...
PROCEDURE upd01.ip:
    ...
    ASSIGN asstf.acct    = wsLedger
           ...
           asstf.amt     = assmf.AValue
           asstf.Descrip = wsdes
           asstf.Per     = wsPer
           asstf.ref     = "Disposal:" + STRING(wsPer)
           asstf.TrDate  = wsDate.
    /* create Cr Asset Ledger */
    FIND FIRST dbgl WHERE dbgl.acct = wsCrLedger AND dbgl.SOURCE   = "AS" 
                      AND dbgl.REF      = "Disposal:" + STRING(wsPer) no-error. 
    IF NOT AVAILABLE dbgl THEN DO:
       CREATE dbgl.
       ASSIGN  dbgl.CREDATE = TODAY
               dbgl.DESCRIP  = wsDes 
               dbgl.period   = wsPer
               dbgl.REF      = "Disposal:" + STRING(wsPer)
               ...
               dbgl.UID2    = varUser.
    END.
    ASSIGN dbgl.AMT     = dbgl.AMT + (Assmf.AValue  * -1).
    ...
    IF NOT AVAILABLE dbgl THEN DO:
       ...
       ASSIGN  dbgl.CREDATE = TODAY
               ...
               dbgl.UID2    = varUser.
    END.
    ASSIGN dbgl.AMT     = dbgl.AMT + ( Assmf.AccumDep   * -1) + Assmf.AValue.
    ASSIGN Assmf.Astatus = "D"
           Assmf.DDate   = wsDate                
           Assmf.DPrice  = wsAmt.  
    {glcon.i}
END.
