...
DEF FRAME frm-main
    ...
    TITLE "EMAIL EXTRACT" VIEW-AS DIALOG-BOX.
...
wsFile = simctr.repDir + "EmailList.csv".
...
PROCEDURE report.ip:
    IF wsSgrp = "" THEN DO:
        ...
        END.
    END.         
    EXPORT STREAM b DELIMITER ',' "Account"     "Name"	"Address1"	"Address2"	"Address3"	"Stand Number"	"Street"	"Email" "Cell".
.
    IF wsChoice = "C" THEN DO: /* by Consumer */
        FOR EACH dbctf WHERE dbctf.cons >= st-Con AND dbctf.cons <= end-Con NO-LOCK.
            FOR EACH dbcmf WHERE dbcmf.dbAcc >= wsStart AND dbcmf.dbAcc <= wsEnd 
               ...
               EXPORT STREAM b DELIMITER ','  dbcmf.dbacc  dbcmf.NAME dbcmf.Add1 dbcmf.add2 dbcmf.Add3 dbcmf.street dbcmf.StandNo dbcmf.emailAdd dbcmf.Cell.
              /* RUN DataExtract-ip.*/
            END.
        END.
    END.
