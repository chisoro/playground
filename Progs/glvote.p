...
&SCOPED-DEFINE wsMsg           "Vote file already exist"
&SCOPED-DEFINE wsTitle           "Vote File Maintenance"
&SCOPED-DEFINE tmptable             glvote
&SCOPED-DEFINE skey                 glvote.Vote
&SCOPED-DEFINE UpdFields      bfr{&tmptable}.DESCRIP ~
                                        COLUMN-LABEL ' Description ':C
&SCOPED-DEFINE tmpFields      bfr{&tmptable}.Vote ~
                                        COLUMN-LABEL ' Vote ':C ~
...
DO:
  IF (btn-ok:LABEL IN  FRAME frm-input) = "Save" THEN DO:
     FIND FIRST bfr{&tmptable} WHERE bfr{&tmptable}.VOTE  = wsid NO-ERROR.
         IF AVAILABLE bfr{&tmptable} THEN DO:
           MESSAGE  "Vote already exist" VIEW-AS ALERT-BOX.
           RETURN NO-APPLY.
        END.
        ELSE DO:
            CREATE bfr{&tmptable}.
            ASSIGN bfr{&tmptable}.vote = wsid
...
ON CHOOSE OF btn-Del DO:
    ...
    ASSIGN wsid = bfr{&tmptable}.Vote.
    FIND FIRST glmf WHERE glmf.Vote  = bfr{&tmptable}.Vote NO-ERROR.
    IF AVAILABLE glmf THEN DO:
        MESSAGE "Vote has related records - cannot be delete"
            VIEW-AS ALERT-BOX.
        RETURN NO-APPLY.
    END.
...
PROCEDURE proc-edit:
    GET CURRENT qry-{&tmptable} EXCLUSIVE-LOCK NO-WAIT.
    ASSIGN wsid = int(bfr{&tmptable}.Vote).
    FIND FIRST bfr{&tmptable} WHERE bfr{&tmptable}.Vote  = wsid NO-ERROR.
