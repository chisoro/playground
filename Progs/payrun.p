/* Program.................payrun.p
   Notes:...... Payroll Calculation run
   Author:.................S. Mawire
*/
SESSION:DATA-ENTRY-RETURN = TRUE.
DEF STREAM a.
DEF VAR w-orientation AS CHAR      INITIAL "PORTRAIT" /*"landscape" */   NO-UNDO.
DEF VAR w-letterhead  AS CHAR      INITIAL ""            NO-UNDO.
DEF VAR w-fontsize    AS INT       INITIAL 10          NO-UNDO.
DEF VAR Gross LIKE paymtf.CurAmt.
DEF VAR YtdGross LIKE paymtf.CurAmt.
DEF VAR Taxable LIKE paymtf.CurAmt.
DEF VAR wsTotal LIKE gross.
DEF VAR YtdTaxable LIKE paymtf.CurAmt.
DEF VAR StdTaxable LIKE paymtf.CurAmt.
DEF VAR NetPay LIKE paymtf.CurAmt.
DEF VAR Deduct LIKE paymtf.CurAmt.
DEF VAR YtdDeduct LIKE paymtf.CurAmt.
DEF VAR Acrual LIKE paymtf.CurAmt.
DEF VAR TaxCredit LIKE paymtf.CurAmt.
DEF VAR YTaxable LIKE Paymtf.CurAmt.
DEF VAR YTax     LIKE Paymtf.CurAmt.
DEF VAR wsAmt    LIKE Paymtf.CurAmt.
DEF VAR curTax  LIKE  Paymtf.CurAmt.
DEF VAR wsF     LIKE payitem.LVar.
DEF VAR st-per LIKE gltdf.period.
DEF VAR wsStatus LIKE payemf.empcode.
DEF VAR st-emp LIKE payemf.empcode.
DEF VAR end-emp LIKE payemf.empcode.
DEF VAR wsName LIKE payemf.sname.
DEF VAR wsSys  LIKE Paysys.Paysys.
DEF VAR wsDate AS DATE.
DEF VAR sufix AS INT.
DEF VAR wsprefix AS INT.
DEF VAR wsledger LIKE glmf.acct.
DEF VAR wsper AS INT.
DEF VAR X AS INT.
DEF SHARED VAR varUser LIKE simusr.usercode.
DEF VAR wsAn AS LOGICAL.

DEF BUTTON btn-Emp   LABEL "START EMPLOYEE".
DEF BUTTON btn-Emp1   LABEL "END EMPLOYEE".
DEF BUTTON btn-close  LABEL "CLOSE".
DEF BUTTON btn-ok     LABEL "Process".
DEF BUTTON btn-Sys   LABEL "Payroll System".


DEF BUFFER bfrPaymtf FOR paymtf.

DEF    QUERY qry-emp FOR Payemf SCROLLING.
DEF BROWSE brw-emp QUERY qry-emp
        DISPLAY payemf.empcode  payemf.SName WIDTH 40 payemf.FName 
        WIDTH 30 WITH 20 DOWN SEPARATORS.

DEFINE FRAME frm-pickemp 
    brw-emp AT ROW 2 COL 5
    skip(0.5)
    btn-ok colon 5
    btn-close colon 60
    with view-as dialog-box keep-tab-order no-validate
         side-labels no-underline three-d SCROLLABLE CENTERED TITLE "Payroll System Selection".

DEF    QUERY qry-Sys FOR Paysys SCROLLING.
DEF BROWSE brw-Sys QUERY qry-Sys
        DISPLAY Paysys.Paysys Paysys.Descrip COLUMN-LABEL "Decsription" 
        WIDTH 60 WITH 20 DOWN SEPARATORS.

DEFINE RECTANGLE rect-1
     EDGE-PIXELS 2 GRAPHIC-EDGE  NO-FILL
     SIZE 80 by 2.3.

DEFINE RECTANGLE rect-2
     EDGE-PIXELS 2 GRAPHIC-EDGE  NO-FILL
     SIZE 80 by 8.5.

DEFINE FRAME frm-main
    SKIP(0.5)
    btn-Sys   COLON 14 NO-LABEL NO-TAB-STOP
    wsSys     NO-LABEL
    Paysys.Descrip NO-LABEL VIEW-AS TEXT SKIP(0.5)
    st-per    COLON 30 LABEL "Payroll Period" VIEW-AS TEXT SKIP(0.5)
    wsDate    COLON 30 LABEL "Run Date" VIEW-AS TEXT SKIP(0.5)
    btn-Emp    COLON 8 NO-LABEL NO-TAB-STOP st-emp NO-LABEL wsname NO-LABEL VIEW-AS TEXT SKIP(0.5)
    btn-Emp1    COLON 10 NO-LABEL NO-TAB-STOP end-emp NO-LABEL SKIP(0.5)
    wsStatus   COLON 20 LABEL "Processing......" VIEW-AS TEXT NO-TAB-STOP
    skip(1.0)
    btn-ok colon 15
    btn-close colon 50
    rect-2 AT ROW 1.4 COL 3
    rect-1 AT ROW 10 COL 3
    WITH 1 DOWN OVERLAY SIDE-LABELS CENTERED THREE-D 
     SIZE 85 BY 13
    TITLE "PAYROLL CALCULATION" VIEW-AS DIALOG-BOX.

DEFINE FRAME frm-pick 
    brw-Sys AT ROW 2 COL 5
    skip(0.5)
    btn-ok colon 5
    btn-close colon 60
    with view-as dialog-box keep-tab-order no-validate
         side-labels no-underline three-d SCROLLABLE CENTERED TITLE "Payroll System Selection".

FORM
    wsName Gross
    HEADER skip(4) "PAYROLL RUN REPORT FOR THE PERIOD:" AT 5
    TRIM(STRING(st-Per)) "Page:" AT 82 TRIM(STRING(PAGE-NUMBER(a))) SKIP(1)
    WITH DOWN STREAM-IO FONT 10 CENTERED NO-BOX WIDTH 132 FRAME frm-rpt.

ON CHOOSE OF btn-sys IN FRAME frm-main
    OR CHOOSE OF btn-sys IN FRAME frm-main
DO:
  VIEW FRAME frm-pick.
  OPEN QUERY qry-sys FOR EACH paysys NO-LOCK.
  ENABLE ALL WITH FRAME frm-pick.
  WAIT-FOR CHOOSE OF btn-close IN FRAME frm-pick 
          OR close of THIS-PROCEDURE IN FRAME frm-pick
          OR CHOOSE OF btn-ok IN FRAME frm-pick 
          OR 'enter':u OF brw-sys
          OR 'mouse-select-dblclick' OF brw-sys.
  CLOSE QUERY qry-sys.
  HIDE FRAME frm-pick.
  APPLY 'tab' TO SELF.
  RETURN. 
END.

ON CHOOSE OF btn-ok IN FRAME frm-pick 
    OR 'enter':u OF brw-sys
    OR 'mouse-select-dblclick' OF brw-sys
DO: 
   GET CURRENT qry-sys NO-LOCK NO-WAIT.
   DISPLAY Paysys.Paysys @ wsSys Paysys.Descrip WITH FRAME frm-main.
   APPLY 'tab' TO SELF.
   APPLY 'tab' TO wsSys IN FRAME frm-main.
END.

ON  'enter':U OF wsSys IN FRAME frm-main
    OR 'tab':U OF wsSys IN FRAME frm-main
    OR 'leave':U OF wsSys IN FRAME frm-main
DO:
    ASSIGN wsSys.
    IF INT(wsSys:SCREEN-VALUE) = 0 THEN
        APPLY 'close' TO THIS-PROCEDURE.
    ELSE DO:
        ASSIGN wsSys.
        FIND FIRST paysys WHERE Paysys.Paysys = INT(wsSys:SCREEN-VALUE) NO-LOCK NO-ERROR.
        IF AVAILABLE paysys THEN DO:
           FIND FIRST simusr WHERE simusr.usercode = varUser NO-LOCK NO-ERROR.
           IF AVAILABLE simusr AND (simusr.Paysys = 99 OR simusr.Paysys = INT(wsSys:SCREEN-VALUE)) THEN
           DO:
              DISPLAY Paysys.Descrip WITH FRAM frm-main.
              ASSIGN st-per:SCREEN-VALUE = STRING(paysys.CURPER)
                     wsDate:SCREEN-VALUE = STRING(Paysys.CurDate)
                     wsper = paysys.period.
              APPLY 'entry' TO btn-ok IN FRAME frm-main.
           END.
           ELSE DO:
              MESSAGE "You are not allowed to access this Payroll...Please try again" VIEW-AS ALERT-BOX.
              RETURN NO-APPLY.
           END.
        END.  
        ELSE DO:
            MESSAGE "Invalid Payroll System...Please try again" VIEW-AS ALERT-BOX.
            RETURN NO-APPLY.
        END.
    END.
    RETURN.
END.

ON CHOOSE OF btn-ok IN FRAME frm-main DO:
     session:set-wait-state("").
     IF (wsSys:SCREEN-VALUE IN FRAME frm-main) = "" THEN DO:
         MESSAGE "Invalid Payroll System....." VIEW-AS ALERT-BOX.
         RETURN NO-APPLY.
     END.
     FIND FIRST Paysys WHERE Paysys.paysys = INT(wsSys:SCREEN-VALUE IN FRAME frm-main) NO-LOCK NO-ERROR.
     ASSIGN st-per = INT(st-per:SCREEN-VALUE IN FRAME frm-main)
            wsDate = DATE(wsDate:SCREEN-VALUE IN FRAME frm-main)
            wsSys  = INT(wsSys:SCREEN-VALUE IN FRAME frm-main)
            st-emp end-emp.
            {PrintOpt.i &stream-name="stream a"
                    &print-prog="Report.ip"
                    &paged}
END.

ON CHOOSE OF btn-EMP IN FRAME frm-main
DO:
  VIEW FRAME frm-pickemp.
  OPEN QUERY qry-emp FOR EACH payemf WHERE payemf.estatus <> 4 AND payemf.paysys = wsSys NO-LOCK.
  ENABLE ALL WITH FRAME frm-pickemp.
  WAIT-FOR CHOOSE OF btn-close IN FRAME frm-pickemp 
          OR close of THIS-PROCEDURE IN FRAME frm-pickemp
          OR CHOOSE OF btn-ok IN FRAME frm-pickemp 
          OR 'enter':u OF brw-emp
          OR 'mouse-select-dblclick' OF brw-emp.
  CLOSE QUERY qry-emp.
  HIDE FRAME frm-pickemp.
  APPLY 'tab' TO SELF.
  DISPLAY payemf.empcode @ st-emp WITH FRAME frm-main .
  APPLY 'tab' TO st-emp IN FRAME frm-main.
  RETURN. 
END.

ON CHOOSE OF btn-EMP1 IN FRAME frm-main
DO:
  VIEW FRAME frm-pickemp.
  OPEN QUERY qry-emp FOR EACH payemf WHERE payemf.estatus <> 4 AND payemf.paysys = wsSys NO-LOCK.
  ENABLE ALL WITH FRAME frm-pickemp.
  WAIT-FOR CHOOSE OF btn-close IN FRAME frm-pickemp 
          OR close of THIS-PROCEDURE IN FRAME frm-pickemp
          OR CHOOSE OF btn-ok IN FRAME frm-pickemp 
          OR 'enter':u OF brw-emp
          OR 'mouse-select-dblclick' OF brw-emp.
  CLOSE QUERY qry-emp.
  HIDE FRAME frm-pickemp.
  APPLY 'tab' TO SELF.
  DISPLAY payemf.empcode @ end-emp WITH FRAME frm-main .
  APPLY 'tab' TO end-emp IN FRAME frm-main.
  RETURN. 
END.

ON CHOOSE OF btn-ok IN FRAME frm-pickemp 
    OR 'enter':u OF brw-emp
    OR 'mouse-select-dblclick' OF brw-emp
DO: 
   GET CURRENT qry-emp NO-LOCK NO-WAIT.
   APPLY 'tab' TO SELF.
   RETURN. 
END.

ON  'enter':U OF st-emp IN FRAME frm-main
    OR 'tab':U OF st-emp  IN FRAME frm-main
DO:
    IF INT(st-emp :SCREEN-VALUE) = 0 THEN DO:
        FIND FIRST payemf WHERE payemf.paysys = wsSys NO-LOCK NO-ERROR. 
        st-emp:SCREEN-VALUE = STRING(payemf.empcode).
    END.
    FIND FIRST payemf WHERE payemf.empcode  = INT(st-emp :SCREEN-VALUE) AND payemf.paysys = wsSys NO-LOCK NO-ERROR.
    IF AVAILABLE payemf AND payemf.estatus = 4 THEN DO:
        MESSAGE "Employee has left...Please try again" VIEW-AS ALERT-BOX.
        RETURN NO-APPLY.
    END.
    IF AVAILABLE payemf  THEN DO:
        wsName = TRIM(payemf.SName) + " " + TRIM(payemf.FName).
        DISPLAY payemf.empcode @ st-emp wsName WITH FRAME frm-main.
    END.
   ELSE IF NOT AVAILABLE payemf AND INT(end-emp:SCREEN-VALUE) <> 0 THEN DO:
        MESSAGE "Invalid Employee code...Please try again" VIEW-AS ALERT-BOX.
        RETURN NO-APPLY.
    END.
    RETURN.
END.

ON  'enter':U OF end-emp IN FRAME frm-main
    OR 'tab':U OF end-emp  IN FRAME frm-main
DO:
    FIND FIRST payemf WHERE payemf.empcode  = INT(end-emp:SCREEN-VALUE) AND payemf.paysys = wsSys NO-LOCK NO-ERROR.
    IF AVAILABLE payemf AND payemf.estatus = 4 THEN DO:
        MESSAGE "Employee has left...Please try again" VIEW-AS ALERT-BOX.
        RETURN NO-APPLY.
    END.
    ELSE IF NOT AVAILABLE payemf AND INT(end-emp:SCREEN-VALUE) <> 9999999 THEN DO:
        MESSAGE "Invalid Employee code...Please try again" VIEW-AS ALERT-BOX.
        RETURN NO-APPLY.
    END.
    RETURN.
END.

/********** MAIN LOGIC **********/

ENABLE ALL WITH FRAME frm-main.
FIND FIRST simctr NO-LOCK NO-ERROR.
FIND FIRST payctr NO-LOCK NO-ERROR.
ASSIGN st-per:SCREEN-VALUE = ""
       wsDate:SCREEN-VALUE = STRING(TODAY)
       st-emp:SCREEN-VALUE = "0"
       end-emp:SCREEN-VALUE = "9999999".
WAIT-FOR CHOOSE OF btn-close OR close of THIS-PROCEDURE IN FRAME frm-main.
HIDE FRAME frm-main.

PROCEDURE Report.ip:
    /* Undo any calculations */
FOR EACH payemf WHERE payemf.paysys = wsSys AND (payemf.estatus = 1 OR payemf.estatus = 2)
                  AND payemf.empcode >= st-emp AND payemf.empcode <= end-emp:
    FOR EACH paymtf WHERE paymtf.empcode = payemf.empcode:
        IF paymtf.curAmt <> 0 THEN
           ASSIGN Paymtf.YtdAmt = Paymtf.YtdAmt - Paymtf.CurAmt
                  Paymtf.CurAmt = 0.
    END.
END.
/* Delete Costing data */
FOR EACH paycost WHERE paycost.paysys = wsSys
                  AND paycost.empcode >= st-emp AND paycost.empcode <= end-emp: /*clean costing data */
    DELETE paycost.
END.
/*Pay calculate */
FOR EACH payemf WHERE payemf.paysys = wsSys AND (payemf.estatus = 1 OR payemf.estatus = 2) 
                 AND payemf.empcode >= st-emp AND payemf.empcode <= end-emp NO-LOCK:
    ASSIGN gross = 0
           taxable = 0
           netpay = 0
           ytdgross = 0
           ytdtaxable = 0
           stdtaxable = 0
           deduct = 0
           ytddeduct = 0
           acrual = 0
           TaxCredit = 0
           wsTotal = 0.
    DISPLAY payemf.empcode @ wsStatus WITH FRAME frm-main.
    IF simctr.LForm = 1 THEN DO:
        FIND FIRST paypost WHERE PayPost.Post = payemf.Post NO-LOCK NO-ERROR.
        FIND FIRST paydept WHERE paydept.dept = payemf.dept NO-LOCK NO-ERROR.
        IF AVAILABLE paydept THEN
            wsprefix = INT(STRING(Paypost.proj) + STRING(Paypost.fund) + string(paydept.prefix)).
    END.
    ELSE IF simctr.LForm = 2 THEN DO:
        FIND FIRST paypost WHERE PayPost.Post = payemf.Post NO-LOCK NO-ERROR.
        wsprefix = 0.
    END.   
    FOR EACH payitem NO-LOCK: 
        IF Payitem.cat = 1 OR Payitem.cat = 3 OR Payitem.cat = 4 THEN /*Income, Acrual and Benefits  costing */
             wsLedger = DEC(TRIM(STRING(wsprefix)) + TRIM(STRING(Payitem.Suffix,"9999"))).
        ELSE wsLedger = 0.
       FIND FIRST paymtf WHERE paymtf.itemcode = payitem.itemcode AND paymtf.empcode = payemf.empcode NO-ERROR.
       IF NOT AVAILABLE paymtf AND payitem.stdAmt = 0 THEN NEXT.
       ELSE DO:
            IF NOT AVAILABLE paymtf AND payitem.stdAmt <> 0 THEN DO:
                CREATE paymtf.
                ASSIGN Paymtf.empcode = payemf.empcode
                       Paymtf.Itemcode = payitem.itemcode
                       Paymtf.ACTIVE  = 1.
            END.
            IF Paymtf.ACTIVE = 1 THEN DO:
                IF payitem.TYPE = 1 AND Paymtf.itemcode = payctr.itemcode THEN DO:
                    ASSIGN Paymtf.CurAmt = paymtf.stdAmt + Paymtf.InpAmt WHEN paymtf.stdAmt <> 0
                           Paymtf.CurAmt = payitem.stdAmt + Paymtf.InpAmt WHEN paymtf.stdAmt = 0
                           Paymtf.YtdAmt = Paymtf.YtdAmt + Paymtf.CurAmt.
                    IF Paymtf.itemcode = payctr.itemcode AND Paymtf.YtdAmt > payemf.leaveMax THEN
                       ASSIGN Paymtf.CurAmt = payemf.leaveMax - Paymtf.YtdAmt
                              Paymtf.YtdAmt = payemf.leaveMax.
                END.   
                ELSE IF payitem.TYPE = 1 AND Paymtf.itemcode <> payctr.itemcode  THEN DO: /* straight Amount */
                    IF  paymtf.stdAmt = 0 AND Payitem.stdAmt <> 0 THEN
                        ASSIGN Paymtf.CurAmt = Payitem.stdAmt WHEN Paymtf.InpAmt = 0
                               Paymtf.CurAmt = Paymtf.InpAmt WHEN Paymtf.InpAmt <> 0.
                    ELSE IF  paymtf.stdAmt <> 0 THEN
                        ASSIGN Paymtf.CurAmt = paymtf.stdAmt WHEN Paymtf.InpAmt = 0
                               Paymtf.CurAmt = Paymtf.InpAmt WHEN Paymtf.InpAmt <> 0.
                        ELSE ASSIGN Paymtf.CurAmt = Paymtf.InpAmt WHEN Paymtf.InpAmt <> 0.
                   IF Paymtf.CurAmt > payitem.ULAmt THEN
                      ASSIGN Paymtf.CurAmt = payitem.ULAmt WHEN payitem.ULAmt <> 0.
                   IF Paymtf.CurAmt < payitem.LLAmt THEN
                      ASSIGN Paymtf.CurAmt = payitem.LLAmt WHEN payitem.LLAmt <> 0.
                   Paymtf.YtdAmt = Paymtf.YtdAmt + Paymtf.CurAmt.
                END. 
                ELSE IF payitem.TYPE = 2 THEN DO: /* Percentage paired */
                    FIND FIRST bfrpaymtf WHERE bfrpaymtf.itemcode = Payitem.Linked 
                        AND bfrpaymtf.empcode = payemf.empcode NO-LOCK NO-ERROR.
                    IF AVAILABLE bfrpaymtf THEN DO:
                       IF  paymtf.stdAmt = 0 AND Payitem.stdAmt <> 0 THEN
                        ASSIGN Paymtf.CurAmt = (Payitem.stdAmt / 100) * bfrpaymtf.curAmt WHEN Paymtf.InpAmt = 0
                               Paymtf.CurAmt = (Paymtf.InpAmt / 100) * bfrpaymtf.curAmt WHEN Paymtf.InpAmt <> 0.
                        ELSE IF  paymtf.stdAmt <> 0 THEN
                            ASSIGN Paymtf.CurAmt = (paymtf.stdAmt / 100) * bfrpaymtf.curAmt WHEN Paymtf.InpAmt = 0
                                   Paymtf.CurAmt = (Paymtf.InpAmt / 100) * bfrpaymtf.curAmt WHEN Paymtf.InpAmt <> 0.
                             ELSE ASSIGN Paymtf.CurAmt = (Paymtf.InpAmt / 100) * bfrpaymtf.curAmt WHEN Paymtf.InpAmt <> 0.
                       IF Paymtf.CurAmt > payitem.ULAmt THEN
                            ASSIGN Paymtf.CurAmt = payitem.ULAmt WHEN payitem.ULAmt <> 0.
                       IF Paymtf.CurAmt < payitem.LLAmt THEN
                            ASSIGN Paymtf.CurAmt = payitem.LLAmt WHEN payitem.LLAmt <> 0.
                       Paymtf.YtdAmt = Paymtf.YtdAmt + Paymtf.CurAmt.
                    END.
                END.
                ELSE IF payitem.TYPE = 3 THEN DO: /* Hours paired */
                    FIND FIRST bfrpaymtf WHERE bfrpaymtf.itemcode = Payitem.Linked 
                        AND bfrpaymtf.empcode = payemf.empcode NO-LOCK NO-ERROR.
                    IF AVAILABLE bfrpaymtf THEN DO:
                        ASSIGN Paymtf.CurAmt = paymtf.stdAmt * bfrpaymtf.CurAmt.
                        IF Paymtf.CurAmt > payitem.ULAmt THEN
                             ASSIGN Paymtf.CurAmt = payitem.ULAmt WHEN payitem.ULAmt <> 0.
                        IF Paymtf.CurAmt < payitem.LLAmt THEN
                             ASSIGN Paymtf.CurAmt = payitem.LLAmt WHEN payitem.LLAmt <> 0.
                        Paymtf.YtdAmt = Paymtf.YtdAmt + Paymtf.CurAmt.
                    END.
                END.
               ELSE IF payitem.TYPE = 4 THEN DO: /* Special item */
                   ASSIGN paymtf.CurAmt = gross * (payitem.Stdamt / 100) WHEN payitem.LVar = "Gross"
                          paymtf.CurAmt = (gross + acrual) * (payitem.Stdamt / 100) WHEN payitem.LVar = "Gross + Acrual".
                    IF Paymtf.CurAmt > payitem.ULAmt THEN
                      ASSIGN Paymtf.CurAmt = payitem.ULAmt WHEN payitem.ULAmt <> 0.
                   IF Paymtf.CurAmt < payitem.LLAmt THEN
                      ASSIGN Paymtf.CurAmt = payitem.LLAmt WHEN payitem.LLAmt <> 0.
                   Paymtf.YtdAmt = Paymtf.YtdAmt + Paymtf.CurAmt.
                END.
                CASE Payitem.cat:
                    WHEN 1 THEN /* Income */
                        ASSIGN Gross      = gross + Paymtf.CurAmt
                               YtdGross   = YtdGross + Paymtf.YtdAmt
                               Taxable    = Taxable + Paymtf.CurAmt    WHEN payitem.tax = 1
                               YtdTaxable = YtdTaxable + Paymtf.YtdAmt WHEN payitem.tax = 1
                               StdTaxable = stdTaxable + Paymtf.CurAmt WHEN (Payitem.Regular = YES AND payitem.tax = 1)
                               NetPay     = NetPay +  Paymtf.CurAmt
                               wsTotal = wsTotal + Paymtf.CurAmt. 
                    WHEN 2 THEN  /* Deduction */
                        ASSIGN Deduct     = Deduct + Paymtf.CurAmt
                               YtdDeduct  = YtdDeduct + Paymtf.YtdAmt
                               Taxable    = Taxable - Paymtf.CurAmt    WHEN payitem.tax = 2
                               YtdTaxable = YtdTaxable - Paymtf.YtdAmt WHEN payitem.tax = 2
                               StdTaxable = stdTaxable - Paymtf.CurAmt WHEN (Payitem.Regular = YES AND payitem.tax = 2)
                               NetPay  = NetPay - Paymtf.CurAmt.
                    WHEN 3 THEN /* acrual */
                        ASSIGN Acrual = acrual + Paymtf.CurAmt WHEN Payitem.Itemcode <> Payctr.Itemcode
                               wsTotal = wsTotal + Paymtf.CurAmt WHEN Payitem.Itemcode <> Payctr.Itemcode.
                    WHEN 4 THEN /* Benefit */
                        ASSIGN Gross      = gross + Paymtf.CurAmt
                               YtdGross   = YtdGross + Paymtf.YtdAmt
                               Taxable    = Taxable + Paymtf.CurAmt    WHEN payitem.tax = 1
                               YtdTaxable = YtdTaxable + Paymtf.YtdAmt WHEN payitem.tax = 1
                               StdTaxable = stdTaxable + Paymtf.CurAmt WHEN (Payitem.Regular = YES AND payitem.tax = 1)
                               wsTotal = wsTotal + Paymtf.CurAmt. 
                    OTHERWISE DO:
                        ASSIGN TaxCredit = TaxCredit + Paymtf.CurAmt WHEN payitem.tax = 3
                               Taxable    = Taxable - Paymtf.CurAmt  WHEN payitem.tax = 2
                               YtdTaxable = YtdTaxable - Paymtf.YtdAmt WHEN payitem.tax = 2.
                    END.         
                END CASE.
        END. /* eof ACTIVE = 1 */
    END.
    IF wsLedger <> 0 AND paymtf.CurAmt <> 0 THEN DO: /* Payroll costing data */
        CREATE paycost.
        ASSIGN paycost.empcode = payemf.empcode
               paycost.itemcode = payitem.itemcode
               paycost.ledger   = wsLedger
               paycost.Proj     = paypost.proj
               paycost.Fund     = paypost.fund
               paycost.dept     = payemf.dept
               paycost.Amount   = paymtf.CurAmt
               paycost.paysys   = wsSys.
    END.
    END. /*eof payitem */
    IF payctr.meth = 1 THEN DO:
        IF payemf.startper <= Paysys.Perclosed + 1  THEN
          RUN FDS.ip.
        ELSE 
           RUN PAYE.ip.
    END.
    ELSE IF payctr.meth = 2 THEN 
       RUN PAYE.ip.
END. /*eof payemf */
RELEASE paycost.
RELEASE paymtf.
APPLY 'CLOSE' TO THIS-PROCEDURE.
RETURN.
END.

PROCEDURE FDS.ip:
       /*DISPLAY STREAM a "Std Pay" @ wsName stdTaxable @ gross WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt.
        DISPLAY STREAM a "Ytd Income" @ wsName YtdTaxable - StdTaxable  @ gross WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt. */
        YTaxable = stdTaxable * (wsper - (Paysys.CurPer - Paysys.PerClosed) + 1). /*Project std Pay */
        
        /*DISPLAY STREAM a "Std Pay" @ wsName yTaxable @ gross WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt. */
        YTaxable = YTaxable + (YtdTaxable - stdTaxable). /*Annualised Taxable */
       /*DISPLAY STREAM a "Annual Taxable" @ wsName yTaxable @ gross WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt. */
        FIND FIRST paytax WHERE PayTax.Taxfrom <= YTaxable AND  PayTax.Taxto >= YTaxable NO-LOCK NO-ERROR.
        YTax = (YTaxable * (PayTax.Tax% / 100)) - PayTax.Deduct.
       /*DISPLAY STREAM a "Annual Tax" @ wsName yTax @ gross WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt. */
        FIND FIRST paymtf WHERE paymtf.empcode = payemf.empcode AND paymtf.itemcode = 1 NO-ERROR.
        IF NOT AVAILABLE paymtf THEN DO:
            CREATE paymtf.
            ASSIGN Paymtf.empcode = payemf.empcode
                   Paymtf.Itemcode = 1
                   Paymtf.ACTIVE  = 1.
        END.
        yTax = yTax / wsper. /* Monthly tax portion */
        /* yTax = yTax - Paymtf.YtdAmt. */
        CurTax = (yTax * (Paysys.CurPer - Paysys.PerClosed)) - Paymtf.YtdAmt. 
        /*curTax =  ROUND((yTax /( wsper - (Paysys.CurPer - Paysys.PerClosed) + 1)),2). */
        /*DISPLAY STREAM a "Required Tax to-date" @ wsName curTax @ gross WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt. */
        ASSIGN curTax = 0 WHEN curTax = ?.
        
        curTax = /* ROUND((curTax - Paymtf.YtdAmt),2) */ CurTax - TaxCredit.
      /* DISPLAY STREAM a "Credits" @ wsName TaxCredit @ gross WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt.
        DISPLAY STREAM a "Current tax" @ wsName curTax @ gross WITH FRAME frm-rpt.
        DOWN STREAM a WITH FRAME frm-rpt. */
        IF curTax < 0 THEN
           curTax = 0.
        ASSIGN Paymtf.CurAmt = curTax
               Paymtf.YtdAmt = Paymtf.YtdAmt + curTax
               netPay = NetPay - curTax.
        
        FIND FIRST payitem WHERE payitem.itemcode = Payctr.AidsLevy NO-LOCK NO-ERROR.
        FIND FIRST paymtf WHERE paymtf.empcode = payemf.empcode AND paymtf.itemcode = Payctr.AidsLevy NO-ERROR.
        IF NOT AVAILABLE paymtf THEN DO:
            CREATE paymtf.
            ASSIGN Paymtf.empcode = payemf.empcode
                   Paymtf.Itemcode = Payctr.AidsLevy
                   Paymtf.ACTIVE  = 1.
        END.
        IF curTax > 0 THEN
        ASSIGN Paymtf.CurAmt = round(curTax * (payitem.stdAmt / 100),2)
               Paymtf.YtdAmt = Paymtf.YtdAmt +  round(curTax * (payitem.stdAmt / 100),2)
               netPay = NetPay - round(curTax * (payitem.stdAmt / 100),2).
END.

PROCEDURE PAYE.ip:
    FIND FIRST paytax WHERE (PayTax.Taxfrom / wsper) <= Taxable AND ( PayTax.Taxto / wsper ) >= Taxable NO-LOCK NO-ERROR.
        curTax = (Taxable * (PayTax.Tax% / 100)) - (PayTax.Deduct / wsper ).
        FIND FIRST paymtf WHERE paymtf.empcode = payemf.empcode AND paymtf.itemcode = 1 NO-ERROR.
        IF NOT AVAILABLE paymtf THEN DO:
            CREATE paymtf.
            ASSIGN Paymtf.empcode = payemf.empcode
                   Paymtf.Itemcode = 1
                   Paymtf.ACTIVE  = 1.
        END.
        curTax = ROUND(curTax,2) - TaxCredit.
        IF curTax < 0 THEN
           curTax = 0.
        ASSIGN Paymtf.CurAmt = curTax
               Paymtf.YtdAmt = Paymtf.YtdAmt + curTax
               netPay = NetPay - curTax.
        FIND FIRST payitem WHERE payitem.itemcode = Payctr.AidsLevy NO-LOCK NO-ERROR.
        FIND FIRST paymtf WHERE paymtf.empcode = payemf.empcode AND paymtf.itemcode = Payctr.AidsLevy NO-ERROR.
        IF NOT AVAILABLE paymtf THEN DO:
            CREATE paymtf.
            ASSIGN Paymtf.empcode = payemf.empcode
                   Paymtf.Itemcode = Payctr.AidsLevy
                   Paymtf.ACTIVE  = 1.
        END.
        IF curTax > 0 THEN
        ASSIGN Paymtf.CurAmt = round(curTax * (payitem.stdAmt / 100),2)
               Paymtf.YtdAmt = Paymtf.YtdAmt +  round(curTax * (payitem.stdAmt / 100),2)
               netPay = NetPay - round(curTax * (payitem.stdAmt / 100),2).
END.
