...
DEF VAR wsCur2   LIKE cbkmf.txtCur.
DEF VAR wsRate LIKE tblForex.decRate.
DEF VAR sAmt    LIKE cbkmf.bal. /*source value*/
...
DEF BUTTON btnBank1     LABEL "CASH ACCOUNT".
DEF BUTTON btnBank2     LABEL " DEPOSIT BANK".
DEF BUTTON btnPrint    LABEL "DEPOSIT".
...
DEF FRAME frm-Input
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER NO-VALIDATE
         ...
         SIZE 100 BY 15.5 TITLE "BANKING OF CASH RECEIPTS".
...
FORM
    ...
    wsAmt         LABEL "AMOUNT " 
    HEADER  "            BANKING OF CASH RECEIPTS          DATE:" wsDate   "   PERIOD :" st-per  
    "    Page: " AT 80 PAGE-NUMBER(a) SKIP(2)
...
/*need to check bank currency. We cannot deposit money from different currencies */
...
DO:
    ...
    ELSE DO:
        ...
        IF NOT AVAIL cbkmf THEN DO:
            ...
            RETURN NO-APPLY.
        END.
        ELSE DO: /*added to read currency*/
...
ON CHOOSE OF btnPrint IN FRAME frm-Input
    /*section to do the conversion*/
DO: 
    FIND first tblForex WHERE tblForex.txtCur = wsCur AND date(wsDate:SCREEN-VALUE) >= tblForex.DtRate  NO-LOCK NO-ERROR.
  ...
  /*this section needs a recheck */
    END.
    IF wsCur = wsCur2 THEN DO:
        wsAmt = DEC(wsAmt:SCREEN-VALUE).
        sAmt = DEC(wsAmt:SCREEN-VALUE).
    END.
    ELSE IF wsCur <> wsCur2 THEN DO:
        sAmt = DEC(wsAmt:SCREEN-VALUE).
        wsAmt = DEC(wsAmt:SCREEN-VALUE) * wsRate.
    END.
       
    ASSIGN st-per = INT(st-per:SCREEN-VALUE)
           wsYear = INT(SUBSTR(STRING(st-per),1,4))
           wsMonth = INT(SUBSTR(STRING(st-per),5,2))
           wsRef wsDate st-per wsbank[1] wsBank[2].
...
PROCEDURE Update-ip:
    ...
    DO X = 1 TO 2:
       ...
       ASSIGN cbkmf.bal = cbkmf.bal - sAmt WHEN X = 1
                  cbkmf.bal = cbkmf.bal + wsAmt WHEN X = 2.
       CREATE bfcbTrans. /* Update and Create Bank transaction */
       ASSIGN bfCBTrans.Accper = st-per
              bfCBTrans.amount = sAmt * -1 WHEN X = 1
              bfCBTrans.amount = wsAmt      WHEN X = 2
              bfCBTrans.bank   = wsBank[X]
              bfCBTrans.Descrip = "Deposit to " + STRING(wsBank[2]) WHEN X = 1
              bfCBTrans.Descrip = "Deposit from " + STRING(wsBank[1]) WHEN X = 2
              bfCBTrans.OpCode  = varUser
              bfCBTrans.Ref     = wsRef
              bfCBTrans.TransID = wsTransId
              bfCBTrans.TranType = X
              bfCBTrans.trDate   = wsDate
              bfCBTrans.ledger   = cbkmf.Ledger
              bfCBTrans.seq      = 0.
       ...
       IF NOT AVAILABLE dbgl THEN DO:
          ...
          ASSIGN  dbgl.CREDATE = TODAY
                  dbgl.DESCRIP  = "Deposit to " + STRING(wsBank[2]) WHEN X = 1
                  dbgl.DESCRIP  = "Deposit from " + STRING(wsBank[1]) WHEN X = 2
                  dbgl.period   = st-per
                  dbgl.REF      = wsRef
                  ...
                  dbgl.SOURCE   = "CB".
       END.
       ASSIGN dbgl.AMT = dbgl.AMT + (bfCBTrans.amount * wsRate).
