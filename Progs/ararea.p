...
/* Program.................arArea.p
   Notes:...... Capture Asset Areaegories
   Author:.................S. Mawire
*/
&SCOPED-DEFINE wsMsg           "Area file already exist"
&SCOPED-DEFINE wsTitle           "Area file Maintenance"
&SCOPED-DEFINE tmptable             area
&SCOPED-DEFINE skey                 area.area
&SCOPED-DEFINE UpdFields      bfr{&tmptable}.DESCRIP ~
                                        COLUMN-LABEL ' Description ':C
&SCOPED-DEFINE tmpFields      bfr{&tmptable}.Area ~
                                        COLUMN-LABEL ' Area ':C ~
...
DO:
  IF (btn-ok:LABEL IN  FRAME frm-input) = "Save" THEN DO:
     FIND FIRST bfr{&tmptable} WHERE bfr{&tmptable}.Area  = wsid NO-ERROR.
         IF AVAILABLE bfr{&tmptable} THEN DO:
           MESSAGE  "Area already exist" VIEW-AS ALERT-BOX.
           RETURN NO-APPLY.
        END.
        ELSE DO:
            CREATE bfr{&tmptable}.
            ASSIGN bfr{&tmptable}.Area = wsid
...
ON CHOOSE OF btn-Del DO:
    ...
    ASSIGN wsid = bfr{&tmptable}.Area.
    FIND FIRST assmf WHERE Assmf.area  = bfr{&tmptable}.Area NO-ERROR.
    IF AVAILABLE assmf THEN DO:
        MESSAGE "Area has related records - cannot be delete"
            VIEW-AS ALERT-BOX.
        RETURN NO-APPLY.
    END.
...
PROCEDURE proc-edit:
    GET CURRENT qry-{&tmptable} EXCLUSIVE-LOCK NO-WAIT.
    ASSIGN wsid = int(bfr{&tmptable}.Area).
    FIND FIRST bfr{&tmptable} WHERE bfr{&tmptable}.Area  = wsid NO-ERROR.
