...
DEF FRAME frm-Input
    SKIP(1)
     ...
     btnOrd    COLON 40  NO-TAB-STOP wsOrd NO-LABEL SKIP(0.2)
...
ON 'tab' of wsOrd IN FRAME frm-Input
    ...
    OR 'leave' OF wsord IN FRAME frm-input
DO:
    ASSIGN wsord.
   IF DEC(wsOrd:SCREEN-VALUE) <> 0 THEN DO:
        ...
        ELSE DO:
            ASSIGN wsNar:SCREEN-VALUE = ordmf.Descrip    
                   wsAmt:SCREEN-VALUE = STRING(ordmf.OrdAmt - ordmf.invAmt).
...
DO:
    ASSIGN wsAmt.
    IF DEC(wsOrd:SCREEN-VALUE) <> 0 AND wsAmt > (ordmf.OrdAmt - ordmf.invAmt) THEN DO:
        MESSAGE "Amount can only be " (ordmf.OrdAmt - ordmf.invAmt) " or less"
            VIEW-AS ALERT-BOX.
        wsAmt:SCREEN-VALUE = STRING(ordmf.OrdAmt - ordmf.invAmt).
        RETURN NO-APPLY.
    END.
...
DO: 
   ...
   ASSIGN crdmf.bal = crdmf.bal + dec(wsAmt:SCREEN-VALUE) * wsFac
          crdmf.vatAmt = crdmf.vatAmt + dec(wsVat:SCREEN-VALUE) * wsFac
          Crdmf.YtdAmt = Crdmf.YtdAmt + dec(wsAmt:SCREEN-VALUE) * wsFac.
...
PROCEDURE per-Order:
    FOR EACH Ordtmf WHERE Ordtmf.ordNo = ordmf.ordNo:
           wsTotal <> 0.00 OR wsTVat <> 0.00. 
              ...
              wsVat = wsVat + tmpTrans.vat.
    END.
    wsVat:SCREEN-VALUE = STRING(wsVat).
    CLOSE QUERY qry-crdTrans.
    OPEN QUERY qry-crdTrans FOR EACH  tmpTrans EXCLUSIVE-LOCK.
    ENABLE btnPrint WITH FRAME frm-Input.
    /*APPLY "CLOSE":U TO THIS-PROCEDURE. */
...
PROCEDURE Update-ip:
   ...
   FOR EACH tmpTrans WHERE tmpTrans.TransID = wsTransId:
      ...
      IF wsOrd <> 0 THEN DO:  /* update and mark Order line status */
          FIND FIRST ordtmf WHERE ordtmf.ordno = wsOrd AND ordtmf.lineSeq = tmpTrans.lineSeq NO-ERROR.
           ordtmf.invAmt = ordtmf.invAmt + tmptrans.Amount.
      END.
      DELETE tmpTrans.
   ...
   IF AVAILABLE ordmf THEN DO:
       ordmf.invAmt = ordmf.invAmt + wsAmt.
