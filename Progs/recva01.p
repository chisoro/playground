...
DEF VAR wsRecNo LIKE bfdbRec.RecNo.
FORM bfdbRec.RecNo      LABEL "RECEIPT"
     bfdbRec.SeqNo      LABEL "SEQ"
     bfdbRec.Account    LABEL "ACCOUNT"
     bfdbRec.Descrip      LABEL "DESCRIPTION" FORM "X(20)"
     bfdbRec.Paytype    LABEL "PCODE"
     bfdbRec.rCode      LABEL "ICODE"
     bfdbRec.Amount     LABEL "AMOUNT" FORM "ZZZZZZZZZZ9.99-"
     wsTotal            LABEL "RECPT TOTAL"
     bfdbRec.txtCur     LABEL "CUR" FORM "x(3)"
     wsStatus           LABEL "COMMENT" FORM "x(7)"
    HEADER wsTitle AT 20
       skip(1) "             RECEIPT VALIDATION REPORT FOR CASHIER: " wsOp " RECEIPTING DATE: " wsDate 
...
PROCEDURE Report.ip.
    FOR EACH simctr:
        repCur = ldCur.
    END.
   /*FOR EACH dbRec where dbRec.OpCode = wsOp AND dbRec.RecDate = wsDate NO-LOCK BREAK BY dbRec.RecNo BY dbRec.seq :
        ASSIGN         
         dbRec.decRate = 1 WHEN dbRec.txtCur = repCur.
    END.*/
   FOR EACH bfdbRec where bfdbRec.OpCode = wsOp AND bfdbRec.RecDate = wsDate NO-LOCK BREAK BY bfdbRec.RecNo BY bfdbRec.seq :
       wsStatus = "".
       wsVar = bfdbRec.rcode.
       
          
       IF bfdbRec.Recstat = "A" THEN
           wsStatus = "Amended".
       ...
       DISPLAY STREAM a bfdbRec.Descrip bfdbRec.Paytype /*wsVar @ */ bfdbRec.rCode bfdbRec.SeqNo bfdbRec.Account 
               bfdbRec.Amount bfdbRec.txtCur wsStatus WITH FRAME frm-rpt.
       /* DOWN STREAM a WITH FRAME frm-rpt. */
       IF LAST-OF(bfdbrec.recno) THEN
   ...
   FOR EACH bfdbRec WHERE bfdbRec.OpCode = wsOp AND bfdbRec.RecDate = wsDate AND bfdbRec.Recstat <> "C"
       ...
       IF LAST-OF(bfdbRec.rCode) THEN DO:
           FIND FIRST dbRcod WHERE dbRCod.rCode =  bfdbRec.rCode NO-LOCK NO-ERROR.
           wsVar = bfdbRec.rCode. 
           DISPLAY STREAM a wsVar @ bfdbRec.Account dbRCod.descrip @ bfdbRec.descrip wsTotal WITH FRAME frm-rpt.
           DOWN STREAM a WITH FRAME frm-rpt.
       END.      
   END.
   UNDERLINE STREAM a wsTotal WITH FRAME frm-rpt.
