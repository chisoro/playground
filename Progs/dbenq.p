...
DEF VAR wsTitle AS CHAR FORM "x(80)" INITIAL "ACCOUNT TRANSACTION HISTORY REPORT FROM PERIOD: ".
...
DEF VAR aStat AS CHAR.

DEF VAR recep LIKE dbRecH.recNo.
DEF VAR wsLasRecTotal LIKE DbRec.Amount FORM "zzzzzzzzz9.99-".
DEF VAR cur AS CHAR.
...
DEF BROWSE brw-trans QUERY qry-trans
    ...
    WIDTH 30 dbTrans.Vat LABEL "VAT" dbTrans.Amt LABEL "AMOUNT"  dbTrans.bf LABEL "BALANCE"
...
DEF BROWSE brw-rec QUERY qry-rec
    DISPLAY dbrec.opcode dbrec.recno dbrec.recdate dbrec.rcode dbrec.amount dbrec.decrate dbrec.txtcur 
    WITH 20 DOWN SEPARATORS.
...
DEFINE FRAME frmWater
    ...
    btn-ok COLON 10 LABEL "PRINT" SPACE(25) btnclose
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER NO-VALIDATE
...
define frame frm-enq
    ...
    dbcmf.street    COLON 15 LABEL "Street Address" VIEW-AS TEXT NO-TAB-STOP  SPACE (18)
    dbcmf.lpDate LABEL "LAST RECEIPT" VIEW-AS TEXT NO-TAB-STOP
    /*wsLasRecTotal NO-LABEL VIEW-AS TEXT NO-TAB-STOP
    cur NO-LABEL VIEW-AS TEXT NO-TAB-STOP SKIP(0.2)*/
    dbsmf.Descrip    COLON 15 LABEL "Suburb" FORM "x(34)" VIEW-AS TEXT NO-TAB-STOP SPACE (5)
    dbcmf.accStat LABEL "ACCOUNT STATUS" VIEW-AS TEXT NO-TAB-STOP
...
FORM /*dbcmf.dbacc  LABEL "ACCOUNT"
     */
    "ACCOUNT NUMBER: " dbcmf.dbacc  NO-LABEL dbcmf.NAME NO-LABEL  SKIP
    "METER NUMBER# " dbmmf.Meter  NO-LABEL  SKIP(1)
    "TARIF# " dbmmf.Tarif NO-LABEL  SPACE(2) dbtmf.Descrip no-label skip(1)
    "LAST 12 METER READINGS" COLON 5 SKIP
    "READING DATE           READING     CONSUMPTION      COMMENTS" COLON 2 SKIP
    dbmmf.RDate[1] AT 8 NO-LABEL space(4) dbmmf.read[1] NO-LABEL space(4) 
    dbmmf.consum[1] NO-LABEL space(3) dbmmf.comm[1] NO-LABEL skip
    ...
    SKIP(1)
    wsTitle2 AT 10
    SKIP(2) with width 132
              NO-BOX stream-io
              no-labels FRAME frm-waterrpt.
...
DEFINE FRAME frm-rec 
    ...
    btnclose colon 15
    with view-as dialog-box keep-tab-order no-validate
         side-labels no-underline overlay three-d SCROLLABLE CENTERED TITLE "UNPOSTED RECEIPTS".

DEFINE FRAME frm-CELL
    SKIP(1.0)
     dbcmf.Cell  COLON 10 LABEL "CELL" NO-TAB-STOP SKIP(0.5)
     dbcmf.emailAdd  COLON 10 LABEL "eMAIL"   SKIP(1)
     btn-ok       COLON 10 LABEL "UPDATE"
     space(20) BtnClose
   with view-as dialog-box keep-tab-order no-validate
         side-labels no-underline overlay three-d SCROLLABLE CENTERED
         CANCEL-BUTTON BtnClose TITLE "CUSTOMER INFORMATION UPDATE".
...
DO:
   ...
   ELSE DO:
      ...
      /* IF CAN-FIND (FIRST landsale WHERE landsale.dbacc = dbcmf.dbacc) THEN
       ...
       ASSIGN aStat = "Active" WHEN dbcmf.accStat = 0
           aStat = "Finalize" WHEN dbcmf.accStat = 95
           aStat = "Closed" when dbcmf.accStat = 99.
         
       DISPLAY dbcmf.dbacc dbcmf.Name dbcmf.stno + "," + dbcmf.street @ dbcmf.street dbcmf.SiteValue dbcmf.Size dbwrd.Descrip dbsmf.Descrip 
           dbcmf.Add1 dbcmf.add2 dbcmf.Add3 dbcmf.BldValue aStat @ dbcmf.accStat dbcmf.lpDate WITH FRAME frm-Enq.
       wsT = 0.
       wsReceipts = 0.
       /*rDate = 01/01/01. */
       wsLasRecTotal = 0.  
     ...
     DISPLAY wsT wsReceipts dbcmf.lpDate /*wsLasRecTotal Cur */ WITH FRAME frm-Enq.
   END.
   RETURN.    
END.
...
DO:
   ...
   /* RUN blfdata-ip.
    FIND FIRST dbwrd WHERE dbwrd.ward = dbcmf.ward NO-LOCK NO-ERROR.
       ...
       DISPLAY dbcmf.dbacc dbcmf.Name dbcmf.stno + "," + dbcmf.street @ dbcmf.street dbcmf.SiteValue dbcmf.Size dbwrd.Descrip dbsmf.Descrip 
           dbcmf.Add1 dbcmf.add2 dbcmf.Add3 dbcmf.BldValue aStat @ dbcmf.accStat WITH FRAME frm-Enq.
...
DO:
  ...
  VIEW FRAME frm-cell.
  DISPLAY dbcmf.cell dbcmf.emailadd WITH FRAME frm-cell.
  ENABLE ALL WITH FRAME frm-cell.
  APPLY 'entry' TO dbcmf.cell.
  WAIT-FOR CHOOSE OF btn-ok OR CHOOSE OF btnclose OR close of THIS-PROCEDURE IN FRAME frm-cell.
  HIDE FRAME frm-cell.
   FIND CURRENT dbcmf NO-LOCK.
  RETURN.
END.

ON 'choose':U OF btn-ok IN FRAME frm-cell 
DO:
    ASSIGN dbcmf.cell dbcmf.emailadd.
    RETURN.
END.
...
ON row-display OF brw-dbEnq DO:
    ...
    ASSIGN wsdbAmt:FGCOLOR IN BROWSE brw-dbEnq = 12 WHEN wsdbAmt < 0
           wsCurr:FGCOLOR  IN BROWSE brw-dbEnq = 12 WHEN wsCurr  < 0
           wsArr:FGCOLOR   IN BROWSE brw-dbEnq = 12 WHEN wsArr   < 0
           wsTotal:FGCOLOR IN BROWSE brw-dbEnq = 12 WHEN wsTotal < 0.
END.

ON ROW-DISPLAY OF brw-trans
DO:
    ASSIGN dbTrans.Amt:FGCOLOR IN BROWSE brw-trans = 12 WHEN dbTrans.Amt < 0 
          dbTrans.Vat:FGCOLOR  IN BROWSE brw-trans = 12 WHEN dbTrans.Vat < 0 
          dbtrans.bf:FGCOLOR   IN BROWSE brw-trans = 12 WHEN dbtrans.bf  < 0.
    RETURN.  
END.
...
DO:
  ...
  if not available dbmmf then do:
      ...
      return no-apply.
       ...
       ENABLE  btn-ok btnclose  WITH FRAME frmWater.        
  end.
  WAIT-FOR CHOOSE OF btn-ok OR CHOOSE OF btnclose IN FRAME frmWater 
          OR CLOSE OF THIS-PROCEDURE IN FRAME frmWater.
  HIDE FRAME frmWater.
  RETURN.
END.

ON 'choose':U OF btn-ok IN FRAME frmWater 
DO:
...
PROCEDURE ViewTrans.ip:
  ...
  if wsFilter = 0 THEN DO:
      FOR EACH dbhtf WHERE dbhtf.dbacc = DEC(dbcmf.dbAcc:SCREEN-VALUE IN FRAME frm-Enq)
                       AND dbhtf.Accper < int(wsPer) NO-LOCK BY dbhtf.trDate BY dbhtf.TransID:
...
wsTitle2 = "". 
wsAmt = AccBal.
/*DISPLAY STREAM a dbcmf.dbacc dbcmf.NAME dbcmf.add1 dbcmf.add2 dbcmf.add3 dbcmf.stno + " " + dbcmf.street @ dbcmf.street
    WITH FRAME frm-rpth.
DOWN STREAM a WITH FRAME frm-rpth.*/
IF wsFilter = 0 THEN DO:
    FOR EACH dbhtf WHERE dbhtf.dbAcc = DEC(dbcmf.dbAcc:SCREEN-VALUE IN FRAME frm-Enq) 
    ...
    case trim(hCol:label):
       ...
       when "Street" then
        do:
           open query qry-dbcmf 
                    FOR EACH dbcmf no-lock 
                        where dbcmf.street = wsSearch:SCREEN-VALUE.
                    END.
    END.
    RETURN.
END.
...
PROCEDURE blfdata-ip:
   wsReceipts = 0.
   /*FOR EACH dbrec WHERE dbRec.Account = dbcmf.dbAcc AND dbRec.RecStat <> "C" NO-LOCK:
        ...
        /*ASSIGN wsReceipts = wsReceipts + ROUND((amount  / (892.6349 / 1)),2) WHEN txtcur = "zwl"
           wsReceipts = wsReceipts + amount WHEN txtcur <> "ZWL". */
    END. */
   ...
   /*display unposted receipts  s. Chisoro*/
        ...
        /*display last receipt s.Chisoro*/
        /*FIND LAST dbRecH WHERE dbRecH.account =  dbcmf.dbAcc AND  dbRecH.RecStat = "" AND dbRecH.contype = "C"  
            AND dbrech.recdate = dbcmf.lpdate NO-LOCK NO-ERROR.
        IF AVAILABLE dbRecH THEN DO:
         wsAcc = dbRecH.account.
         recep = dbRecH.recNo.
         wsLasRecTotal = 0.
         cur = dbRecH.txtCur.
         rDate = dbRecH.recDate.
         FOR EACH dbRecH WHERE dbRecH.account = wsAcc AND dbRecH.recNo = recep NO-LOCK.
               wsLasRecTotal = wsLasRecTotal + dbRech.amount.
   ...
   FOR EACH bfrdbblf WHERE bfrdbblf.dbacc = DEC(dbcmf.dbacc:SCREEN-VALUE IN FRAME frm-Enq ): 
    ...
    wsT[4] = wsT[4] - ROUND((wsReceipts * wsRate),2). /*change to display correct zwl value*/
END.

PROCEDURE Water.ip:
    FIND CURRENT dbcmf NO-LOCK NO-ERROR.
    FIND CURRENT dbmmf NO-LOCK NO-ERROR.
    IF dbmmf.mstat = 3 OR dbmmf.tarif = 0 THEN DO:
        MESSAGE "Meter must be closed... cannot be printed" VIEW-AS ALERT-BOX.
        RETURN NO-APPLY.
    END.
    ELSE DO:
        FIND FIRST dbtmf WHERE dbtmf.TYPE = 1 AND dbtmf.tarif = dbmmf.tarif NO-LOCK NO-ERROR.
        DISPLAY STREAM a  dbcmf.dbAcc dbcmf.NAME dbmmf.Meter dbmmf.Tarif dbtmf.Descrip dbmmf.READ 
                          dbmmf.rdate dbmmf.consum WITH FRAME frm-waterrpt.
        DOWN STREAM a WITH FRAME frm-waterrpt.
    END.
