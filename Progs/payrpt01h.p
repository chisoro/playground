...
/* Program.................payrpt01h.p
    Notes:...... ..........Historical Payroll Analysis Report
    Author:.................S. Mawire
*/
DEF STREAM a.
...
DEFINE RECTANGLE rect-2
 ...
 FORM SPACE(10)
    ...
    HEADER wsCo "PAGE" AT 90 PAGE-NUMBER(a) SKIP(1) 
          "PAYROLL ANALYSIS FROM HISTORT FOR THE PERIOD ENDED " wsDate SKIP(2)
 ...
 DEFINE FRAME frm-Paysys
     ...
     wsDate    COLON 26 LABEL "PAYROLL DATE"  SKIP(2)
     btn-ok colon 20
     btn-close colon 60
     ...
     WITH 1 DOWN OVERLAY SIDE-LABELS CENTERED THREE-D 
      SIZE 95 BY 9
     TITLE "PAYROLL ANALYSIS REPORT FROM HISTORY" VIEW-AS DIALOG-BOX.
...
PROCEDURE Report.ip:
    ASSIGN wsAmt = 0
           wsCnt = 0.
FOR EACH payhtf WHERE payhtf.trdate = wsDate AND CurAmt <> 0 BREAK BY Itemcode:
    FIND FIRST payemf WHERE payemf.empcode = payhtf.empcode NO-LOCK NO-ERROR.
    IF payemf.Paysys <> wsSys THEN NEXT.
    ACCUMULATE payhtf.CurAmt (TOTAL BY Itemcode)
               payhtf.CurAmt (COUNT BY Itemcode).
    IF FIRST-OF(payhtf.Itemcode) THEN
        wsAmt = 0.
    IF LAST-OF(itemcode) THEN DO:
        FIND FIRST payitem WHERE payitem.itemcode = payhtf.itemcode NO-ERROR.
        IF AVAILABLE payitem  AND payitem.cat <> 0 THEN DO:
            ASSIGN wsAmt[1] = (ACCUM TOTAL BY payhtf.Itemcode CurAmt) WHEN Payitem.cat = 1 OR Payitem.cat = 4
                   wsAmt[2] = (ACCUM TOTAL BY payhtf.Itemcode CurAmt) WHEN Payitem.cat = 2
                   wsAmt[3] = (ACCUM TOTAL BY payhtf.Itemcode CurAmt) WHEN Payitem.cat = 3 
                   wscnt    = (ACCUM COUNT BY payhtf.Itemcode CurAmt) 
