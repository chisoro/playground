...
df = pd.read_csv(fs, engine ='python',  dtype ={'AMOUNT': float}, encoding= 'unicode_escape')
#df = df1[df1['Email'] != 'nan']
#df = pd.read_csv(fs, low_memory=False)
...
for i in range(len(pivot)):
    ...
    def create_pdf():
        ...
        c.setFont("Helvetica", 14)
        if(regn != 'nan'):
            c.drawString(350,710,"REG. NUMBER: " + str(regn))
        if(bpn != 'nan'):
            c.drawString(350,695,"BP NUMBER: " + str(bpn))
        if(elmail != 'nan'):
    ...
    if validate_email(str(pivot.loc[i, "Email"])):
        send_email(subject, body, sender, recipients, password, server, port)
    else:
        print(pivot.loc[i, "Name"], pivot.loc[i, "Account"], "Message not sent! Invalid email address. " + str(pivot.loc[i, "Email"]))
